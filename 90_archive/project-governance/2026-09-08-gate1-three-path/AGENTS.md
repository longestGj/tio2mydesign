...
总控编排（状态恢复、任务准入、派发、接收、协调及阶段关闭）使用[Project Orchestrator角色](agents/project-orchestrator/agent.md)及其三个引用合同；按[生效增补](docs/architecture/PROJECT_ORCHESTRATOR_ACTIVATION_V1.4.md)解释治理细则§6.2与阶段交接。总控按各Gate承担必要判断，不默认重复有效独立审查；具体页面授权和批准方式保持。
