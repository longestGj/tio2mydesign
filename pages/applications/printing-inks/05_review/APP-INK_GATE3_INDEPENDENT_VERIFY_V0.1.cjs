...
const base='D:/23MySec/pages/applications/printing-inks/04_planning/gate3-v0.1';
const out='D:/23MySec/pages/applications/printing-inks/05_review/APP-INK_GATE3_INDEPENDENT_OBSERVATIONS_V0.1.json';
const read=p=>fs.readFileSync(p,'utf8'),json=p=>JSON.parse(read(p)),hash=b=>crypto.createHash('sha256').update(b).digest('hex');
const freeze=json(base+'/APP-INK_GATE3_FREEZE_RECORD_V0.1.json'),inventory=json(base+'/APP-INK_GATE3_ASSET_INVENTORY_V0.1.json');
...
collect(freeze);collect(inventory);collect(json(base+'/APP-INK_GATE3_PREFLIGHT_RECORD_V0.1.json'));
...
for(const p of ['APP-INK_GATE3_FREEZE_RECORD_V0.1.json','APP-INK_GATE3_ASSET_INVENTORY_V0.1.json']){const b=fs.readFileSync(base+'/'+p);result[p]={bytes:b.length,sha256:hash(b)}}
...
const b=read(freeze.approvedInputs.B.path).match(/<!-- BUYER_COPY_START -->([\s\S]*?)<!-- BUYER_COPY_END -->/)[1],expected=marked.parse(b);
