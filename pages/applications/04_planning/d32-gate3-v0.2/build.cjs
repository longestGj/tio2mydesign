...
const collections = [
  {
    ...
    ownerPageId: 'APP-COAT',
    href: '/applications/titanium-dioxide-for-coatings/'
  },
  {
    key: 'plastics',
    name: 'Plastics',
  ...
  },
  {
    ...
    ownerPageId: 'APP-INK',
    href: '/applications/titanium-dioxide-for-printing-inks/'
  },
  {
    key: 'paper',
    name: 'Paper',
    ...
    ownerPageId: 'APP-PAPER',
    href: '/applications/titanium-dioxide-for-paper/'
  },
  {
...
const collectionMarkup = (collection) => `<article class="applicationCard" id="application-${esc(collection.key)}" data-application-anchor data-collection>
  ...
  ${collection.action ? `<a class="textAction applicationAction" data-application-action data-owner-page-id="${esc(collection.ownerPageId)}" href="${esc(collection.href)}">${esc(collection.action)}</a>` : ''}
</article>`;
...
const extraction = {
  ...
  collections: collections.map(({ key, name, scope, grades, action, ownerPageId, href }) => ({
    key,
    name,
    scope,
    grades,
    action,
    ...(action ? { owner_page_id: ownerPageId, href } : {})
  })),
  steps,
  procurement,
  application_action_readiness: 'LOCAL_PLANNING_SIMULATION_ONLY: explicit owner Page ID and route mappings preserve the provisional target-readiness boundary and do not claim that a target route is live.',
