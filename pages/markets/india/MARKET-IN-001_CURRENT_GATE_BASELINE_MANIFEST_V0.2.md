# MARKET-IN-001 Current Gate Baseline Manifest V0.2
...
| Review ID / Status | MARKET-IN-001-G1-01 / APPROVED / CLOSED |
| Current Gate | Gate 1 research and intent confirmation |
| Content checkpoint | CONTENT_INTENT_CONFIRMED=CONFIRMED |
| User approval date | 2026-09-06；原话“继续”，在总控逐项呈现IN-D01/IN-D02和下一步之后 |
| Approval scope | IN-D01方向/CTA + IN-D02精确COO句与同义可见/SEO/GEO/social/Schema；不授权Gate2 |
| Next Gate | Gate 2 NOT_STARTED / NOT_AUTHORIZED; a separate Controller owns Gate 2 |

执行授权为用户逐页完成Gate1、一个确认一个，以及本轮确认Belgium后“继续”的India派发。执行子代理使用项目Agent与真实Skill，自检提交不等于总控复审或用户批准。本V0.2是当前唯一组合Manifest；V0.1保留为已被本版本取代的审查阶段历史，不能并列当作当前批准合同。首次执行前未发现独立India旧Brief/单页Gate1包。既有2026-08-29全站研究是上游资料，不是本页已批准成果。
...
|---|---|---:|---|
| Complete Brief | docs/page-briefs/MARKET-IN-001_INDIA_BRIEF_V0.2.md | 11800 | 7D7476413D0E6E3F7C22638A99BDD3927D72000173C77728CEE7FE37855ADB2F |
| Agent synthesis | pages/markets/india/01_research/MARKET-IN-001_GATE1_RESEARCH_REPORT_V0.2.md | 13483 | 1278D66FD18A168C8BF2E1791E2915EE7A955328493796022057A235F6BD4FD3 |
| Claim / evidence register | pages/markets/india/01_research/MARKET-IN-001_GATE1_CLAIM_EVIDENCE_REGISTER_V0.2.csv | 10273 | 520F8316DFECEB93A6880D0A19B060EF4000E53970FF980571CEBFEB5B8A7855 |
| Current combination / governance | pages/markets/india/MARKET-IN-001_CURRENT_GATE_BASELINE_MANIFEST_V0.2.md | self | no self-hash |

Brief §2是八项已确认页面意图卡，§6是两项用户批准记录；研究负责复用/缺口、发现与反例、优先级、owner和后续输入；CSV共有20个稳定命题ID，区分项目事实、观察、推论、用户批准和外部实时结论。此包没有最终整页文案或模块序，不替代Gate2。
...
消费顺序依根AGENTS：最新明确用户决定 → 已批准具体页面合同 → 当前PRD及增补 → 实施关键词主表 → Playbook → 有效证据 → 研究 → 历史。本版本记录本页Gate1批准，不扩大其他授权；Context/Index仅导航。主要来源：
...
- `docs/architecture/MALAYSIA_ORIGIN_SITE_WIDE_PUBLICATION_AUTHORITY_V1.0.md`；`docs/architecture/EVIDENCE_GAP_USER_DECISION_REGISTER_V1.6.md`。Malaysia-origin全站授权直接继承；V1.6 EG-002已正式登记India COO批准，与用户2026-09-06原话“继续”及Brief V0.2 §6一致；不重复询问或扩张批准范围。
...
| ID | 精确批准对象 | 当前状态 |
|---|---|---|
| IN-D01 | 面向印度工业TiO2采购/进口和技术评估人员，重点照顾涂料、塑料、母粒制造；以Malaysia-origin供应与产品评估为先，用型号、用途、MT数量和目的地准备可比较询价；主CTA Request a Quote，次CTA Explore Products；贸易细节转RES-TRADE-IN | USER_APPROVED / 2026-09-06 / 用户原话“继续” |
| IN-D02 | “A Certificate of Origin is available upon request.”（可按申请提供原产地证书）用于India正文及语义一致的SEO/GEO/social/Schema | USER_APPROVED / 2026-09-06 / 用户原话“继续” |

总控在对话直接呈现上述两项及下一步后，用户回复“继续”；依据逐页确认合同登记两项USER_APPROVED。不要重复询问origin、企业身份或已确认India COO。批准该句不扩张为每批自动附证、具体海关待遇或税率结论。
...
自检包括四核心/支持资料读回、CSV结构及来源、raw哈希、矩阵统计/哈希、状态一致、冲突标记和行尾空白。总控先按§3历史独立审查记录通过研究，再由用户2026-09-06“继续”确认两项；执行者据此同步APPROVED / CLOSED，不以自检代替批准。

当前不得进入Gate2；Gate1已APPROVED / CLOSED、CONTENT_INTENT_CONFIRMED=CONFIRMED，下一Controller仍需独立Gate2授权。本次确认不授权开发/上线。未建立Brazil、子页、脚本、代码或测试；未修改共享入口/主表/矩阵；未操作 `D:/16Wordpress_nextjs`。
...
| 2026-09-06 | V0.2 | 用户原话“继续”确认IN-D01/IN-D02，批准四核心；历史V0.1和独立review/raw不改 | APPROVED / CLOSED |

## 8. 历史保留映射

以下V0.1仅为历史审查阶段记录，不再作为当前执行权威；方法、raw、独立review仍作为V0.2支持证据原样引用。

| Historical role | 文件 | Bytes | SHA-256 |
|---|---|---:|---|
...
| Historical Manifest | pages/markets/india/MARKET-IN-001_CURRENT_GATE_BASELINE_MANIFEST_V0.1.md | 8554 | 145E29960981F7913D6FAE2CAD6F3239ADA75100707D83ED0606FBC87045EC02 |
