...
按[准备合同](../../../docs/architecture/WEBSITE_PREPARATION_HANDOFF_CONTRACT_V1.0.md)以site_id和preparation_scope恢复原实例。先复用当前来源，定位W1–5差额；接收专业结果与独立链，再记录用户决定和首批就绪范围。网站基线只保存引用。无授权不启动页面；新增页面不重跑整站准备；并行工作继续按原授权和实际槽位执行。
