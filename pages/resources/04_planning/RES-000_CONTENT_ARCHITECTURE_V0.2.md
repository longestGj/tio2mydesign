# RES-000 Content Architecture V0.2 — Procurement Research Hub Revision
...
The former route-safe zero-inventory page was structurally valid but over-weighted site navigation and risk language. V0.2 makes the Hub itself useful through direct procurement guidance, one real child-page candidate, concrete research paths and extractable buyer answers. It does not invent content to fill the layout.
...
| Description | `Use a practical due-diligence framework to evaluate alternative-origin titanium dioxide supply through origin evidence, grade-specific technical documentation, application requirements and destination-market context.` | Neutral content draft; no capability/equivalence claim |
...
The V0.4 full visual displays this one proposed card for review. Development delivery must not render or link it until the RES-ORIGIN atomic public gate passes.
...
| `02 / TECHNICAL EVALUATION` | `Technical evaluation` | `Use technical research to evaluate titanium dioxide grade characteristics, comparison criteria, application requirements and relevant processing considerations. A research framework does not select a grade.` |
...
|---|---|---|
| BQ-01 | `What should buyers compare when evaluating titanium dioxide grades?` | `Buyers should compare the intended application, relevant process classification, grade-specific technical data, defined performance criteria, technical documentation and destination-market requirements. Values should be reviewed from current documents using comparable test methods, then validated in the buyer’s formulation or processing system.` |
...
- Desktop: 12-column, max 1200px; Featured one-card editorial width, then three equal decision-path cards.
...
| ID | Status | Meaning |
|---|---|---|
| RES-HUB-RESTRUCTURE-01 | `DRAFT_FOR_PROJECT_CONTROL_REVIEW` | Directed Gate 2–5 structural revision |
...
| RES-G2-01 | `REOPENED_FOR_DIRECTED_REVISION / OPEN_USER_APPROVAL` | Prior Gate 2 pending state is superseded for body review only |

Self-check does not approve Gate 2 or any child page.
