...
| 本版定向登记 | 正式登记 Desktop/Mobile RFQ 差异、PRODUCT V0.3 边界，以及用户批准的 manufacturer 可见关系与 SEO/GEO 合同 |
| 继承范围 | V0.2 已批准的页面职责、URL、主关键词、模块顺序、视觉与事实边界全部继承 |
| 阶段权限 | Gate 5 已批准；Gate 6 总控复审已通过；仍不授权 Gate 7、开发、部署或发布 |

本文件在 V0.2 基线上只记录当前已批准的响应式 RFQ 内容差异、PRODUCT V0.3 证据边界，以及本轮获授权的 manufacturer 可见关系和 SEO/GEO 合同，不重新开启页面职责、URL、主关键词、模块顺序或视觉决策。V0.2 与所有历史视觉文件继续保留，不删除、不覆盖。
...
5. `pages/home/04_planning/HOME-001_SEO_GEO_SCHEMA_CONTRACT_V0.1.md`（`HOME-G6-SEO-GEO-PCR-02 / PROJECT_CONTROL_REVIEW_PASS / CLOSED`）
...
11. 用户于 2026-08-31 批准响应式内容差异：Desktop 在 Resources / Buyer Answers 后保留页面级结构化 RFQ Section；Mobile 删除该 Section，Resources / Buyer Answers 后直接进入 Footer。Mobile 的 Request a Quote 入口仍固定存在于 Header、Hero、Mobile Menu 和 Footer，因此该差异不是漏模块，也不改变 RFQ 页的页面职责。
12. 用户于 2026-08-31 明确批准 `manufacturer` 可公开出现。首页 Company 模块可使用最小句：`TiO₂ Malaysia is operated by IKHLAS TITANIUM (MALAYSIA) SDN. BHD., a titanium dioxide manufacturer.`。该批准不证明工厂位置、Malaysia 制造产地、COO、产能、认证或追溯。
13. 首页 SEO/GEO 合同锁定 Title、Meta Description、Canonical、robots、language、当前 H1，以及 WebSite、WebPage、Organization、Brand 和轻量 generic Product 图谱。最终关系为 WebPage→isPartOf→WebSite、WebSite→publisher→Organization、WebPage→about→Brand/Organization/Product、Product→brand→Brand、Product→manufacturer→Organization；不使用 Organization→brand。`manufacturer` 不得挂在 WebPage、WebSite 或 Organization。Review ID：`HOME-G6-SEO-GEO-PCR-02`。
...
Gate 5 已由用户批准并关闭
→ Gate 6 总控定向复验已 PASS
→ HOME-G6-SEO-GEO-PCR-02 已 PROJECT_CONTROL_REVIEW_PASS / CLOSED
→ 等待用户是否授权 Gate 7
```

当前只允许完成本次授权的共享 Chrome 状态板、Mobile Products Expanded、Tablet 响应式证据、SEO/GEO 合同和最小 manufacturer 可见句同步；不得修改生产代码、创建开发交付或进入 `D:\16Wordpress_nextjs`。
...
|---|---|---|---|
| About | `malaysia titanium dioxide manufacturer` | 使用获批的最小 manufacturer 关系句说明平台与运营主体，并链接 About | 详细证明制造地点、Malaysia origin、COO、产能、认证、质量和追溯 |
...
GEO 作为内容表达层，不创建新的首页关键词所有权。首页各核心模块可使用简短、直接、可引用的答案句，并明确链接到事实 owner 页面；可见正文与结构化数据必须一致。当前合同只允许稳定 `@id` 的 WebSite、WebPage、Organization、Brand 和轻量 generic Product；WebSite.publisher 与 Product.manufacturer 指向 IKHLAS Organization，Product.brand 指向 TiO₂ Malaysia Brand，WebPage.about 连接 Brand/Organization/Product。publisher 只表示网站内容与运营责任，不证明所有权、母公司、工厂、地点、原产地或公司控制关系。没有证据的答案不生成，也不使用 Schema 放大未核验主张。

### B.5 SEO / GEO / Schema 锁定字段
...
| robots | `index, follow` |
| language | `EN` |
| H1 | `Malaysia Titanium Dioxide for Industrial Buyers`（保持已批准视觉，不重写） |

完整 JSON-LD、字段证据映射、禁止字段和后续开放条件见 `HOME-001_SEO_GEO_SCHEMA_CONTRACT_V0.1.md`。合同已通过 Gate 6 项目总控复验，但尚未实施，Gate 7 仍待用户授权。
...
| 2 | `[Hero：Malaysia TiO₂ 采购入口]` | 在首屏建立首页主题、受众和第一行动 | 这是什么供应平台？是否服务工业采购？下一步是什么？ | `malaysia titanium dioxide`；supplier 语义 | 1 个 H1 主题；2–3 句说明性占位；主要受众/范围；不展开 origin 证明 | RFQ、Products | Request a Quote | View Products（锁定） | 品牌关系、可公开供应定位、受众范围 | 只写中性平台定位；manufacturer 只在 Company 模块使用获批最小句；不写 stable supply、lead time 或证书承诺；不得以 Request Documents 替代次 CTA |
...
| 5 | `[Products：14 Grades 与选型路径]` | 明确产品深度，并把买家导向 Hub、分组、工艺和型号页 | 有多少型号？我如何按应用或工艺选型？ | `titanium dioxide pigment`、rutile、grades、chloride/sulfate 概述 | 明示 `14 grades`；四组紧凑文字/标签/展开结构；桌面端显示全部 14 个型号 ID；不制作 14 张卡片或 14 段详情 | Products Hub、两个工艺页、14 型号页 | View All 14 Grades（唯一主 CTA） | Chloride Process / Sulfate Process（文本路径） | PRODUCT V0.3 产品关系基线 | 只显示已批准型号和中性关系；M-2377 可按 PRODUCT V0.3 中性显示，`NO_PUBLIC_MAPPING` 静默不渲染；M-996/M-2196 不作比较 |
...
| 7 | `[About 摘要：Malaysia Origin 与运营主体]` | 提供信任入口，但把证明职责留给 About | 谁在运营？Malaysia origin 如何进一步核实？ | broad origin value；不主攻 manufacturer 词 | 获批句：`TiO₂ Malaysia is operated by IKHLAS TITANIUM (MALAYSIA) SDN. BHD., a titanium dioxide manufacturer.`；继续导向 About | `/about/`、Contact | Learn About TiO₂ Malaysia | Contact | 用户对 manufacturer 的明确批准；注册主体与品牌关系基线 | 该句不扩展为工厂地点、Malaysia manufacturing、COO、产能、认证或追溯事实；其他未验证字段不渲染 |
...
| Inks & Multi-Application Grades | M-52、M-2377 | 2 | 依据 PRODUCT V0.3 中性显示两个型号；不发布排名、替代、性能或未批准比较结论 |
...
- 首页主转化。
- Desktop 出现在 Header、Hero、页面级 Final RFQ 和 Footer；Mobile 出现在 Header、Hero、Mobile Menu 和 Footer。
- Mobile 不渲染页面级 Final RFQ Section，Resources / Buyer Answers 后直接进入 Footer；这是用户于 2026-08-31 批准的响应式内容差异，不是遗漏、故障或待恢复状态。
...
7. 页面级 Final RFQ 以字段摘要和 `Request a Quote` 收口。
...
| Final RFQ | 不渲染；Resources / Buyer Answers 后直接进入 Footer。RFQ 继续由 Header、Hero、Mobile Menu 和 Footer 固定承接，不添加替代 CTA Section，不显示 Sample |

移动端目标不是机械缩小或机械纵向堆叠桌面页，而是保留：网站是什么、服务哪个市场、有哪些产品路径、如何核实、如何询价。页面级 Final RFQ 的删除不减少 RFQ 可发现性，因为 Header、Hero、Mobile Menu 与 Footer 仍固定保留 RFQ。核心入口不得依赖无限横向滑动；移动导航、模块压缩和可控页面长度均为响应式硬验收项。
...
| 已确认工艺分类 | PRODUCT V0.3：8 Chloride、5 Sulfate（含 M-2377）、1 Vapor-phase oxidation | `PRODUCT_GRADE_APPLICATION_PROCESS_MATRIX_V0.3.csv` |
| 顶层导航与页面所有权 | Home、Markets、Products、Applications、Documents、Resources、About、RFQ | PRD V0.4 / 主表 |
| 运营主体的 manufacturer 关系 | `IKHLAS TITANIUM (MALAYSIA) SDN. BHD., a titanium dioxide manufacturer` | 用户明确批准，2026-08-31；仅支持可见 Company 句与 `Product.manufacturer` |
| Home SEO/GEO 合同字段 | Title、Meta、Canonical、robots、EN、当前 H1、最小实体图谱 | `HOME-001_SEO_GEO_SCHEMA_CONTRACT_V0.1.md`；Gate 6 定向复审草案 |

说明：获批的 `manufacturer` 关系不等于工厂位于 Malaysia，也不等于具体型号的产地、COO、批次或交易事实已经核验。正式 origin、工厂和履约声明仍需证据登记。

### K.2 `pending_verification`

以下内容未获得资料支持前不得形成正式首页文案：

- IKHLAS 与具体工厂主体、工厂地点、Malaysia 制造地点之间的准确法律/运营关系；IKHLAS 的通用 titanium dioxide manufacturer 表述已单独获批，不再列为待核。
...
- M-996 与 M-2196 的差异、排名、优越性、等效替代或比较式选型；所有未在 PRODUCT V0.3 中获准的产品性能和推荐。
...
- [x] M-2377 依据 PRODUCT V0.3 中性归入 Sulfate，并可中性映射 Coatings、Plastics、Masterbatch、Printing Inks、Paper；Specialty Materials 继续 `NO_PUBLIC_MAPPING / DO_NOT_RENDER`。
...
- [x] 用户批准的 manufacturer 最小可见句已与 Product→Organization Schema 合法关系同步；未扩展为制造地点、Malaysia origin、COO、产能或认证。
- [x] Title、Meta Description、Canonical、robots、language、当前 H1 和最小实体图谱已写入 Home SEO/GEO 合同草案。
- [x] 本 V0.3 不改变已批准首页正文、模块顺序、URL、主关键词或视觉方向；新增画面只用于本次 Gate 6 定向复审证据。
...
- [x] Desktop 保留页面级 Final RFQ；Mobile 不渲染该 Section，Resources / Buyer Answers 后直接进入 Footer，RFQ 仍由 Header、Hero、Mobile Menu 与 Footer承接。
- [x] `manufacturer` 可公开出现，但只使用用户批准的 Company 最小句；Schema 仅允许 `Product.manufacturer` 指向 IKHLAS Organization。
- [x] SEO/GEO P0 与 Review ID `HOME-G6-SEO-GEO-PCR-02` 已通过项目总控复验并关闭；这不等于实施完成或用户已授权 Gate 7。
...
- [x] Gate 5 已关闭；Gate 6 项目总控复审已通过；未自行进入 Gate 7。

### M.3 仍需业务证据或后续 PRD 解决的事项

- [ ] 业务证据：具体工厂主体、生产地点、品牌/运营/工厂关系，以及 Made in Malaysia、manufactured in Malaysia 的批准表述；通用 manufacturer 关系已获批，不再阻塞本轮合同。
...
- [ ] 产品证据：M-996 与 M-2196 差异及其他未在 PRODUCT V0.3 获准的型号性能、比较与推荐关系。
...
- [ ] 阶段授权：Gate 6 项目总控复审已通过；仍等待用户是否明确授权 Gate 7。RFQ route/form、planned href、production SVG、事实门禁和实现验收项继续开放。
