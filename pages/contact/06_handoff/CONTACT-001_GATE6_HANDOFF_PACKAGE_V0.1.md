# CONTACT-001 Gate 6 Development Handoff Package V0.1
...
Every content query, route resolution, cache key/invalidation path, menu/current-state mapping, SEO/head lookup, form configuration/payload routing, shared component lookup and media/logo resolution must require `site_scope=tio2-my`. A missing or wrong scope fails closed; it never reads or renders content, receiver configuration, contact data, assets or form behavior from TIOVAR, a generic/global scope or another site.
...
These IDs are permanent for package `CONTACT-001-G6-HANDOFF-20260910-01`. A correction may add subchecks but must not renumber or reuse an ID. Each evidence item states repo/branch/commit/evidence HEAD, build/runtime identity, environment/URL, timestamp, inputs/steps, expected and observed result, and artifact path/hash.

| ID | Source | Prerequisite / input | Expected observable result | Required actual evidence | Failure impact | Owner / stage |
|---|---|---|---|---|---|---|
| `CONTACT-G9-01` | Brief §§1,5; C §§1,8–9 | `tio2-my` positive fixture; missing/wrong-scope fixtures; `/contact/` | Clean route resolves as CONTACT-001/200; query/route/cache/menu/SEO/form/media remain scoped; wrong/missing scope yields no plausible cross-site fallback | Response/head capture, scoped CMS/API readback, cold/warm/invalidation trace, positive/negative fixture results | Blocks Gate 9 and release | Platform + CONTACT-001 / Gate 8–9 |
...
| `CONTACT-G9-16` | Gate 8→9 contract | Completed implementation and evidence set | Schema-valid evidence manifest binds package and `CONTACT-G9-01–16` to reproducible repo/build/runtime/evidence identities, hashes, commands, open items and held runtime | Validator/preflight output, hash/link/ID coverage report, git/build/runtime identity and evidence inventory | Blocks Gate 9 admission | Gate 8 developer + Controller reception |
