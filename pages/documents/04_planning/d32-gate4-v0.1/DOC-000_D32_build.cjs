...
const seoSource=fs.readFileSync(path.join(root,'pages/documents/06_handoff/DOC-000_GATE7_SEO_GEO_SCHEMA_SOCIAL_CONTRACT_V0.1.md'),'utf8');
const seoTitle=seoSource.match(/\| `<title>` \/ SEO title \| `([^`]+)`/)[1]; // Approved by current Gate7 Manifest V0.2 and publication overlay §4; not inferred from H1.
...
const html=`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>${esc(seoTitle)}</title><style>${chrome.style}</style><link rel="stylesheet" href="DOC-000_D32_structure.css"></head><body>${chrome.header}${body}${chrome.footer}${chrome.dialog}<script>${chrome.behavior}</script><script src="DOC-000_D32_behavior.js"></script></body></html>`;
