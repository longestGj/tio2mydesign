...
状态：INSTALLABLE；完整母版已完成隔离验证，安装能力限本地文件初始化。尚未安装到真实新站，也未验证真实网站开发结果。
...
- templates：最小交接，不是已批准成果；配置/锁定/初始化工具单独按版本交付，不修改角色方法。
...
本目录独立于原项目角色基线；来源与逐项变更对照保存于框架建设项目，不作为新站运行依赖。release-manifest.json记录本目录准确身份，本版本仍须遵守实例接入合并及真实任务授权。当前不是Codex全局Skill安装或自动注册。
