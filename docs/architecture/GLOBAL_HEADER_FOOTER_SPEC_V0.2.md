# TiO2 Malaysia Global Header / Footer Specification V0.2

## 1. Document Control

| Field | Value |
|---|---|
| Document ID | GLOBAL-CHROME-002 |
| Version | V0.2 |
| Date | 2026-08-30 |
| Current Status | APPROVED_GLOBAL_CHROME_BASELINE |
| Scope | TiO2 Malaysia 全站公开页面的 Header、Mobile Header、Mobile Menu 与 Footer |
| Supersedes for current execution | `GLOBAL_HEADER_FOOTER_SPEC_V0.1.md` |
| Historical baseline | V0.1 保留，不覆盖、不删除 |
| User decision source | 用户于 2026-08-30 明确确认执行“RFQ 全站固定规则”并批准本 V0.2 书面规范 |
...
4. Footer 固定显示 Conversion / `Request a Quote` 行动。
...
2. Explore 导航。
3. Information 导航。
4. Conversion / `Request a Quote` 行动。
5. Copyright。
...
- Footer 不得因页面 route、inventory、support、market 或资料状态而隐藏 Conversion 区域或留下空白带。
...
- 本 V0.2 已获用户批准，当前状态为 `APPROVED_GLOBAL_CHROME_BASELINE`；V0.1 为历史批准基线。
- 页面视觉任务现可按本 V0.2 重导出 Header / Mobile Menu / Footer。
- 每个页面只允许修改 Global Chrome 与由其高度变化引起的画布边界；正文像素、模块顺序、文案与门禁必须保持。
- 修改后的页面成果仍需项目总控复审和用户最终视觉批准。
