...
const target=formal?out+'/approval_core':__dirname+'/candidate-v0.1-r3';fs.mkdirSync(target,{recursive:true});
...
try{
 for(const width of [1440,768,390]){
  ...
  if(!formal){await page.emulateMedia({reducedMotion:'reduce'});check(width+' reduced motion',await page.evaluate(()=>matchMedia('(prefers-reduced-motion: reduce)').matches),null);if(width===1440){await page.setViewportSize({width:720,height:450});await settle();const z=await page.evaluate(()=>({scroll:document.documentElement.scrollWidth,viewport:innerWidth}));check('Desktop 200 percent equivalent layout viewport (simulation only)',z.scroll<=z.viewport,z);}}
