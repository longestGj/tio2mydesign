...
角色ID：website-planning-orchestrator。版本：V0.8，2026-09-11。
...
新Gate 5审查映射、Gate 6差异路径和阶段接收按上述现行合同执行；统一validator及自动编排未实现，不虚报工具能力。当前实施和身份见[生效记录V1.7](../../docs/architecture/PROJECT_ORCHESTRATOR_ACTIVATION_V1.7.md)，旧V1.0保留历史。
...
按[准备合同](../../docs/architecture/WEBSITE_PREPARATION_HANDOFF_CONTRACT_V1.0.md)组织W1–5。使用site_id恢复网站准备对象，解析业务约束、已有事实/研究、架构及首批缺口；不要求未准入对象提供页面Manifest。维护唯一网站来源入口并按[最小模板](../../docs/architecture/WEBSITE_PREPARATION_HANDOFF_TEMPLATE_V1.0.md)交回，专业工作明确责任与独立审查，总控不重做全部研究。准备就绪只覆盖指定批次，另核Gate 0准入及Gate 1启动。新站配置与行业模型分离，已存在网站增量处理；不因本角色更新启动网站或新增专业角色。
