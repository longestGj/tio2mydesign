...
先完整读取本文件和[PROJECT_CONTEXT](PROJECT_CONTEXT.md)，涉及任何Gate的解释、执行、审查、交接或Agent/Skill设计，另必读[当前Gate工作流](docs/architecture/GATE_WORKFLOW_V3.0.md)；Gate 3另按[当前标准§4.3](docs/architecture/PAGE_GATE_1_4_STANDARD_V2.1.md)及[Agent/Skill当前基线](docs/architecture/GATE3_AGENT_SKILL_CURRENT_BASELINE_MANIFEST_V1.4.md)执行，旧规范相冲突的Gate 3制作/检查范围按该定向修订解释；再按第4节读取任务资料；本轮已读且未变的不重复加载。进入子目录执行前读取适用AGENTS；链接和子目录规则不假设自动加载。
...
| Gate 1–4 | [PAGE_GATE_1_4_STANDARD_V2.1.md](docs/architecture/PAGE_GATE_1_4_STANDARD_V2.1.md)适用阶段及共同规则；细则§9；已批准上游成果。G1先复用Context四环节已有输入，只补本页缺口 |
