# MARKET-BR-PT Gate 1 Research Report V0.1

## 0. Control and method handoff

- Page: `MARKET-BR-PT`, Brazil, PT-BR, `/pt-br/markets/brazil/` as `PROVISIONAL_URL`, P1.
...
- Review ID: `MARKET-BR-PT-G1-01`; independent project-control review passed, user decisions pending.
- Lifecycle: `BRIEF_IN_REVIEW`; Gate 2 is not started or authorized.
...
|---|---|---|
| Direct reuse | Registered Page ID, PT-BR, P1, primary keyword, cluster and provisional path | Identity is fixed for Gate 1 research; path permanence is not yet approved |
...
| Decision gap | Exact Portuguese COO sentence | `BR-PT-D02` is presented rather than inherited from English |
| Decision gap | PT-BR path/localization direction | `BR-PT-D03` keeps URL/canonical/hreflang governance explicit |
...
The proposed direction is recorded as `BR-PT-D01=PENDING_USER_CONFIRMATION`. It becomes an approved Gate 1 direction only after independent project-control review and explicit user confirmation.

## 4. Local terminology, reader questions and answer obligations

### 4.1 Terminology decisions proposed for Gate 2
...
6. Is the Certificate of Origin available on request? This requires `BR-PT-D02`.
...
- If `BR-PT-D03` is approved, `/pt-br/markets/brazil/` becomes the planning path. Self-canonical and reciprocal `en`/`pt-BR` hreflang remain implementation and live-pair acceptance work, not a Gate 1 claim.
...
### Exact Portuguese fact still requiring confirmation

`O Certificado de Origem está disponível mediante solicitação.`

This is `BR-PT-D02=PENDING_USER_FACT_CONFIRMATION`. It is natural Portuguese and accurately mirrors the English meaning, but it is not automatically approved by the English-page decision.
...
|---|---|---|---|
| `BR-PT-D01` | Proposed Page Intent Card, CTA language and answer priorities | `PENDING_USER_CONFIRMATION` | Project control presents the exact direction; approval freezes Gate 1 editorial direction only |
| `BR-PT-D02` | Exact sentence `O Certificado de Origem está disponível mediante solicitação.` | `PENDING_USER_FACT_CONFIRMATION` | Approve separately for this Portuguese page and semantically equivalent machine-readable use |
| `BR-PT-D03` | PT-BR counterpart at `/pt-br/markets/brazil/`; later canonical/hreflang paired only when both pages are approved and live | `PENDING_USER_LOCALIZATION_DECISION` | User decides the path/language direction; Gate 1 does not make it live |
...
1. **Direction clear:** proposed audience, problem, answer, CTA and ownership are explicit, while approval remains pending.
...
5. **Localization is page-specific:** natural PT-BR terms, exact Portuguese COO and provisional path are separated from English approvals.
6. **Ready for review:** the four core deliverables and Skill outputs are mapped by the unique Current Manifest.

## 10. Stop condition

Stop at `PROJECT_CONTROL_REVIEW_PASS_PENDING_USER_APPROVAL` and wait for BR-PT-D01/D02/D03. Do not mark `APPROVED / CLOSED`, enter Gate 2, build code, implement canonical/hreflang or operate `D:/16Wordpress_nextjs`.
