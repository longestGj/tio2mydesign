# GRADE-M896 Template Adaptation and Channel Recommendation V0.2

## 1. Control

| Field | Value |
|---|---|
| Decision candidate ID | `GRADE-M896-TEMPLATE-CHANNEL-02` |
| Page ID | `GRADE-M896` |
| Date | `2026-09-02` |
...
| Hero topology | One H1, required four-key neutral `hero.visual`, and exactly two eligible actions: Quote + Sample; Documents remains body-only | Existing Product Detail Hero supports the same four-key visual object and two-action topology | PASS |
...
| Technical | 11 rows using `property + value + testMethod`; Property / Value / Test method | Shared technical-row contract needs one backward-compatible generalization while preserving the approved component and visual language | PASS WITH GATE 8 CONTRACT REQUIREMENT |
...
- Hero visual strings remain neutral and Buyer Clean: no filename-only version/date, origin, factory, packaging, stock or shipment implication.

## 6. Gate 8 one-time backward-compatible technical-row requirement

The shared Product Detail technical-row DTO, renderer and Product JSON-LD mapper must be generalized once to support both shapes:

- M-896 current shape: `property` + explicit `value` + optional `testMethod`;
- M-350/M-510 legacy shape: `property` + optional `standard` + `typical`.

The renderer maps visible columns from each contract’s `technical.columns`. For M-896, the Product `additionalProperty.value` must come from `row.value`; `row.testMethod` appears only as the visible third column or an optional description and must never substitute for the property value. The legacy mapper continues to use `row.typical` for M-350/M-510. Missing value-bearing fields must fail closed rather than emit an empty cell or invalid Schema property.

Gate 8 acceptance must include unit, integration and E2E evidence that M-896 renders all 11 values and methods correctly, Schema uses all 11 `value` strings, and M-350/M-510 visual output, HTML, JSON-LD and screenshots are unchanged.
