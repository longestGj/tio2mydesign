# GRADE-M896 SEO, GEO and Schema Contract V0.2

## 1. Control

| Field | Value |
|---|---|
| Contract ID | `GRADE-M896-SEO-GEO-SCHEMA-02` |
| Page ID | `GRADE-M896` |
| Date | `2026-09-02` |
...
## 7. Hero visual and technical-row projection

- The Hero uses the approved neutral document/evaluation visual treatment with four required text keys: `label`, `technicalFile`, `currentData` and `note`.
- These four strings must match the visible Hero contract. They must not expose the filename-only `V3 2023` token or imply origin, factory, packaging, stock or shipment.
- The visual is not a product photograph and does not authorize Product Schema `image`.
- M-896 technical rows retain `property + value + testMethod`; `value` is the Product `additionalProperty.value` source.
- `testMethod` is visible in the third HTML column and may optionally be included as a description. It must never replace the property value.
- The shared renderer must also continue accepting the M-350/M-510 legacy `property + optional standard + typical` shape without changing their HTML, JSON-LD or screenshots.
