...
> 2026-09-21 当前开发仓库为 `D:\32NextJS` 静态单站，D23负责策划、交付与只读验收。旧D16及D32 WordPress仅作历史，当前按[Gate 8规则](docs/architecture/GATE8_CURRENT_RULES_V1.0.md)和[静态单站决定](docs/architecture/STATIC_NEXTJS_SINGLE_SITE_AND_GOOGLE_INDEXING_DECISION_V1.0.md)执行。

## 1. 范围与启动

适用 `D:\23MySec`：研究、内容/页面架构、视觉、SEO/GEO、开发交付规格与只读验收。静态站代码、测试实现、修复、合并、部署及发布归独立开发项目 `D:\32NextJS`；D23任务不实施这些操作，也不再向 `D:\16Wordpress_nextjs` 派发本网站开发。
...
总控编排（状态恢复、任务准入、派发、接收、协调及阶段关闭）使用[Project Orchestrator角色](agents/project-orchestrator/agent.md)及其三个引用合同；按[生效增补](docs/architecture/PROJECT_ORCHESTRATOR_ACTIVATION_V1.15.md)解释治理细则§6.2与阶段交接。总控按各Gate承担必要判断，不默认重复有效独立审查；具体页面授权和批准方式保持。
...
- 当前流程：`Gate 0 → 1 → 2 → 4制作 → 5独立审查 → 总控关闭 → 6 → 8 → 9 → 10`。历史5/7制作/交付编号仅作历史兼容；新Gate 5独立审查按下述交接合同映射；职责、内部检查点、交接与停止条件由Index指向的当前工作流统一维护。不得跳过有效Gate或把提交、自检、定时运行视为批准；已明确授权的连续流程按原范围执行。
- 页面必须有登记身份和批准 Brief；不得增加页面/URL/主关键词或侵占其他页职责。新页面做URL、主词与意图去重；`NO_PRIMARY_KEYWORD`不强造商业主词。Schema仅表达与批准可见内容一致的事实。
- 制作与核验使用本阶段标准：Gate 4统一负责跨页一致性、内容/操作结构、列数、响应式布局及完整视觉，Gate 5独立审查同一最终组合，Gate 6核对跨合同与开发交付，Gate 9验证实际实现。额外制作或复核须对应要求、变化、缺口或风险；不减免有效独立审查、最低证据或页面合同。暂不调用付费Superdesign。
...
- 当前静态单站不要求多站scope；开发与验收核对批准内容、路由、媒体、共享消费与SEO。只读QA发现问题发给开发任务，不直接修代码。验收通过≠已发布。
- 变更页面、事实、架构、关键词、导航、交付隔离或发布条件，先列原因、影响、风险、回退和同步文件并获批准。保留用户编辑和历史；批准范围变化升版本，不覆盖历史批准源，不删除资料。正式成果不放临时目录。

Gate 4制作与新Gate 5审查交接，必读[共用交接合同](docs/architecture/GATE4_GATE5_VISUAL_HANDOFF_CONTRACT_V1.1.md)：新Gate 5承接合并Gate 4的唯一最终独立审查，按合同映射至现有生命周期及常设关闭；不追加审查/批准轮次。历史Gate 5制作编号仍按旧语义保留，总控维护唯一当前Manifest。

新Gate 5→Gate 6派发与接收必读[接收合同](docs/architecture/GATE5_GATE6_ACCEPTANCE_CONTRACT_V1.1.md)：Controller提供批准组合、有效审查链、关闭与启动依据、开放项和变化；Gate 6核对准入，异常按原责任方返回，不增加接收Gate或重复审查。
...
Gate 6派发、执行、独立复核及交接另必读[职责边界决定V1.0](docs/architecture/GATE6_RESPONSIBILITY_BOUNDARY_DECISION_V1.0.md)，与现行执行合同共同消费：明确批准结果和验收依据，技术实现由Gate 8决定，实际验收归Gate 9。当前Agent/Skill身份从Index读取。

Gate 8相关派发、承接及Gate 9接收必读[当前规则入口](docs/architecture/GATE8_CURRENT_RULES_V1.0.md)；不建立独立Gate 8 Agent。旧角色/方法中的开发目录、CMS/scope及机器交接措辞按该入口解释。

## 4. 按任务读取，而不是全库通读

按下表必读适用章节全文及所要求资料。[治理细则](docs/architecture/PROJECT_GOVERNANCE_DETAILS_V1.6.md)保留原章编号。
...
| Gate 6总审与交付或Gate 9只读QA | 细则§8–17、当前批准Manifest、验收与开放项；另读[Gate8→Gate9机器交接合同](docs/architecture/GATE8_GATE9_EVIDENCE_HANDOFF_CONTRACT_V1.1.md)，QA核对开发提交、证据Manifest和可访问结果。不得把未来运行时依赖当作Gate 1–4自动否决项 |
...
历史Gate 3/4的跨页一致性与布局职责曾按[用户批准增补V1.0](docs/architecture/GATE3_CROSS_PAGE_CONSISTENCY_DECISION_V1.0.md)执行：Gate 3明确页面家族、共享组件、同类模块和必要差异；Gate 4确定列数、具体视觉及响应式布局并复验。旧通用列数锁定按该增补解释；已有专属批准和共享owner约束仍按原范围继承。当前方法组合由Index指向的基线统一维护。

2026-09-20上述决定已同步到执行正文。后续Gate 3/4任务按Index读取当前角色、方法和交接；原先仅靠增补覆盖的V1.13组合保留历史，当时Gate 3基线V1.14与Gate 4基线V1.10保存历史组合；现行合并基线从Index读取。
