...
for(const width of [1440,768,390]){
 ...
 if(width!==1440){
  ...
  await page.locator('.menuButton').click();await page.setViewportSize({width:1440,height:1000});await page.waitForFunction(()=>document.querySelector('.mobileNav').hidden&&!document.querySelector('main').inert&&!document.querySelector('footer').inert);check(`${width}:menu breakpoint closes`,await page.locator('.mobileNav').evaluate(e=>e.hidden));await page.setViewportSize({width,height:1000});
 }
