...
第3步工具候选。母版仍为不可发布候选；下一步对真实母版进行完整隔离验证。工具与已冻结母版分开存放，不修改第2步身份。
...
同一文件系统临时目录用于组装；写入前再次检查计划，最终目录存在就停止。失败不会留下半套正式.workflow；普通异常由TemporaryDirectory清理自己的暂存目录。系统断电可能留下.workflow-stage-*，需核对后清理，不把其当安装完成。并发不是数据库事务；外部规则在最终检查后仍可能变化，阶段启动必须重新核对原始规则。第4步再专门验证故障恢复及链接攻击等边界。
...
验证命令：`python -B -m unittest discover -s tests -v`。第3步7项真实文件系统测试已通过；真实母版的端到端隔离正反例留给第4步。
