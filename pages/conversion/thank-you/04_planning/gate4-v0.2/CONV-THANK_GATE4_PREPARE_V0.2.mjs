...
const out=`${root}/pages/conversion/thank-you/04_planning/gate4-v0.2`;
const workset='CONV-THANK-G4-WORKSET-20260908-02';
...
const inputs=[
 ...
 'pages/conversion/thank-you/05_review/gate5-independent-v0.1/CONV-THANK_GATE5_INDEPENDENT_VISUAL_REVIEW_V0.1.md','pages/conversion/thank-you/04_planning/gate4-v0.1/CONV-THANK_GATE4_EDITABLE_SOURCE_V0.1.html','pages/conversion/thank-you/04_planning/gate4-v0.1/CONV-THANK_GATE4_EVIDENCE_INDEX_V0.1.json'
].map(f=>item(`${root}/${f}`));
...
fs.writeFileSync(`${out}/CONV-THANK_GATE4_INPUT_INDEX_V0.2.json`,JSON.stringify({workset_id:workset,executor:'/root/conv_thank_gate4_execute',inputs,dependencies,informational_navigation_only:['01_PROJECT_INDEX.md','00_PROJECT_STATUS.md']},null,2));
const source=item(`${out}/CONV-THANK_GATE4_EDITABLE_SOURCE_V0.2.html`);
fs.writeFileSync(`${out}/CONV-THANK_GATE4_DESIGN_SOURCE_V0.2.json`,JSON.stringify({workset_id:workset,status:'VISUAL_DIRECTION_CHECKED / TARGETED_REPAIR',source,derived_from:item(`${root}/pages/conversion/thank-you/04_planning/gate4-v0.1/CONV-THANK_GATE4_EDITABLE_SOURCE_V0.1.html`),finding:'CONVTHANK-G5-IR-F01',change:'Footer DOM sequence only: footerGrid → copyright → legalUtilities',direction:{inherited:'V0.1 restrained B2B result utility unchanged',locked:['exact copy','four mutually exclusive states','visual rules','approved request key','state eligibility','action labels and targets','shared behavior','navigation NONE']},media:'NO_MEDIA_INTENDED',decision:'TARGETED_REPAIR / CONTINUE_TO_PREFLIGHT'},null,2));
