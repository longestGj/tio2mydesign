# RES-TRADE-UK Gate 2 Content Contract V0.3
...
| Buyer-copy source | `RES-TRADE-UK_GATE2_FULL_BUYER_CLEAN_COPY_V0.3.md` |
| Date | 2026-09-07 |
...
| Supporting | `Request a Quote` | `/request-a-quote/` | Accepts a known Product / Grade or `Not sure / Need help`. Buyer Copy asks: `Tell us your application, required or estimated quantity in metric tonnes, destination and any other context you already know.` After submission, the team reviews the requirements and details and contacts the buyer using the information provided. This receiver does not recommend a Grade, decide technical suitability, or determine classification, origin, tariff, VAT, remedy or clearance. |

Implementation validation remains unchanged: `Required Quantity` must receive a positive numeric value in metric tonnes. That validation rule is not rendered in Buyer Copy as `positive estimate`.
