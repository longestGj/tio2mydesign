...
const widths=mode==='targeted'?[320]:[1440,1024,768,390,320],height=w=>w===390?844:w===320?812:w===768?1024:1000;
const source='RES-000_D32_GATE4.html';
async function snap(p,name,fullPage=true){if(mode!=='formal')return;const f=path.join(out,name);await p.screenshot({path:f,fullPage});const png=fs.readFileSync(f);assets.push({path:f.replaceAll('\\','/'),bytes:png.length,sha256:sha(f),width:png.readUInt32BE(16),height:png.readUInt32BE(20),viewport:p.viewportSize(),DPR:await p.evaluate(()=>devicePixelRatio),fullPage,source_path:decodeURIComponent(new URL(p.url()).pathname).replace(/^\/(\w:)/,'$1'),source_sha256:sha(decodeURIComponent(new URL(p.url()).pathname).replace(/^\/(\w:)/,'$1')),role:name.includes('READABLE')?'DIAGNOSTIC_SUPPORT':'FORMAL',state:name.includes('LOCAL_SIMULATION')?'LOCAL_SIMULATION':name.includes('MENU')?'MENU_OPEN':name.includes('COOKIE')?'COOKIE_OPEN':name.includes('FOCUS')?'FOCUS':name.includes('HOVER')?'HOVER':'H0_DEFAULT'});}
...
(async()=>{
 ...
 const pageFor=async(w,f)=>{const p=await browser.newPage({viewport:{width:w,height:height(w)},deviceScaleFactor:1,reducedMotion:'reduce'});await p.goto(pathToFileURL(path.join(base,f)).href);await p.evaluate(()=>document.fonts.ready);return p};
 ...
 for(const w of widths){
  ...
  }else{await clean(p);await snap(p,`RES-000_D32_GATE4_H0_${w}.png`);if(w===320){const total=await p.evaluate(()=>document.documentElement.scrollHeight);for(let y=0,n=1;y<total;y+=672,n++){await p.evaluate(t=>scrollTo(0,t),Math.min(y,total-812));await p.evaluate(()=>new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r))));await snap(p,`RES-000_D32_GATE4_READABLE_H0_320_${n}.png`,false)}await clean(p)}if(w===768||w===390||w===320){await p.locator('.menuButton').click();await snap(p,`RES-000_D32_GATE4_MENU_${w}.png`,false);await p.keyboard.press('Escape')}if(w===1440){await p.locator('.hero-action').hover();await snap(p,'RES-000_D32_GATE4_HOVER_1440.png',false);await clean(p)}if(w===390){await p.locator('.hero-action').focus();await snap(p,'RES-000_D32_GATE4_FOCUS_390.png',false);await clean(p);await p.locator('.res-faq').scrollIntoViewIfNeeded();await snap(p,'RES-000_D32_GATE4_FAQ_390.png',false);await p.locator('#cookie-trigger').click();await snap(p,'RES-000_D32_GATE4_COOKIE_390.png',false);await p.keyboard.press('Escape')}}
  await p.close();
 }
 for(const state of ['H2','H4'])for(const w of (mode==='targeted'?[320]:state==='H2'?[1440,390,320]:widths)){
  ...
  }else{await clean(p);await snap(p,`RES-000_D32_GATE4_${state}_${w}_LOCAL_SIMULATION.png`);if(w===320){const sel=state==='H2'?'#featured-resources':'[data-fixture="trade"]';await p.locator(sel).evaluate(e=>scrollTo(0,e.getBoundingClientRect().top+scrollY-35));await snap(p,`RES-000_D32_GATE4_READABLE_${state}_320_TOP_LOCAL_SIMULATION.png`,false);await p.locator(sel+' a').last().scrollIntoViewIfNeeded();await snap(p,`RES-000_D32_GATE4_READABLE_${state}_320_BOTTOM_LOCAL_SIMULATION.png`,false)}}await p.close();
 }
 if(mode!=='formal'){
  ...
  for(const f of [source,'RES-000_D32_H2_LOCAL_SIMULATION.html','RES-000_D32_H4_LOCAL_SIMULATION.html']){const oldFile=f===source?'RES-000_D32_GATE3.html':f;check('unchanged_full_markup_'+f,fs.readFileSync(path.join(base,f),'utf8').replace('RES-000_D32_GATE4.css','RES-000_D32_GATE3.css').trimEnd()===fs.readFileSync(path.join(old,oldFile),'utf8').trimEnd())}
 }
 const browserVersion=browser.version();await browser.close();const r={workset_id:'RES-000-D32-G4-001',author:'/root/res_root_d32',mode,evidenceTypes:['ACTUAL_RUNTIME','SOURCE_INSPECTION','LOCAL_SIMULATION'],source:sha(path.join(base,source)),browser:'Playwright Chromium headless',browserVersion,rendering:{widths,heightPolicy:'1440/1024=1000,768=1024,390=844,320=812',DPR:1,fonts:'local shared Inter Variable; fonts.ready',reducedMotion:'reduce'},DPR:1,checks,observations,assets,passed:checks.filter(c=>c.pass).length,failed:checks.filter(c=>!c.pass).length};
