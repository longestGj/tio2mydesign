...
- 制作与核验使用本阶段标准：Gate 3负责跨页一致性与内容/操作结构，Gate 4确定列数、响应式布局并完成完整视觉，Gate 6核对跨合同与开发交付，Gate 9验证实际实现。额外制作或复核须对应要求、变化、缺口或风险；不减免有效独立审查、最低证据或页面合同。暂不调用付费Superdesign。
...
Gate 3/4的跨页一致性与布局职责按[用户批准增补V1.0](docs/architecture/GATE3_CROSS_PAGE_CONSISTENCY_DECISION_V1.0.md)执行：Gate 3明确页面家族、共享组件、同类模块和必要差异；Gate 4确定列数、具体视觉及响应式布局并复验。旧通用列数锁定按该增补解释；已有专属批准和共享owner约束仍按原范围继承。当前方法组合由Index指向的基线统一维护。

2026-09-20上述决定已同步到执行正文。后续Gate 3/4任务按Index读取当前角色、方法和交接；原先仅靠增补覆盖的V1.13组合保留历史，当前Gate 3基线V1.14与Gate 4基线V1.10绑定实际源身份。
