...
总控编排（状态恢复、任务准入、派发、接收、协调及阶段关闭）使用[Project Orchestrator角色](agents/project-orchestrator/agent.md)及其三个引用合同；按[生效增补](docs/architecture/PROJECT_ORCHESTRATOR_ACTIVATION_V1.16.md)解释治理细则§6.2与阶段交接。总控按各Gate承担必要判断，不默认重复有效独立审查；具体页面授权和批准方式保持。
...
Gate 8相关派发、承接及Gate 9接收必读[开发流程](D:/32NextJS/CONTRIBUTING.md)；不建立独立Gate 8 Agent。旧角色/方法中的开发目录、CMS/scope及机器交接措辞按该入口解释。

## 4. 按任务读取，而不是全库通读

按下表必读适用章节全文及所要求资料。[治理细则](docs/architecture/PROJECT_GOVERNANCE_DETAILS_V1.7.md)保留原章编号。
...
| Gate 6总审与交付或Gate 9只读QA | 细则§8–17、当前批准Manifest、验收与开放项；另读[Gate8→Gate9机器交接合同](docs/architecture/GATE8_GATE9_EVIDENCE_HANDOFF_CONTRACT_V1.2.md)，QA核对开发提交、证据Manifest和可访问结果。不得把未来运行时依赖当作Gate 1–4自动否决项 |
...
2026-09-21用户纠正：开发规则使用D32现有AGENTS.md、CONTRIBUTING.md和README.md，不另建Gate 8规则入口。`GATE8_CURRENT_RULES_V1.0.md`撤出现行规则，仅保留历史；旧文件引用它时，开发事项直接读取上述开发文档，验收证据交换按当前共用交接合同。
