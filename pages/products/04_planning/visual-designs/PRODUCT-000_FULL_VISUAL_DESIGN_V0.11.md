# PRODUCT-000 Full Visual Design V0.11 — Informative Grade Directory Revision
...
| Version | `V0.11` |
| Date | `2026-08-31` |
| Review ID | `PRODUCT-G5-BUYER-FINAL-UX-03` |
...
| Output set | Two complete V0.15 buyer-facing pages: 1440px Desktop and 390px logical Mobile at 2× |
| Scope | Directed directory-information repair only: add a neutral application/process description to every grade row |
...
This revision does not redesign the page or replace the approved global system. It preserves the project-control-approved V0.13 Hero/directory geometry and the user-directed compact Evaluation layout from V0.14, then implements the user's 2026-08-31 request to make every directory row informative: each of the fourteen grades now shows a concise application-and-process description before `View Grade`. Desktop uses a three-zone row; Mobile uses a grade/action first line and description second line. Header/Footer, fixed RFQ, `PROCUREMENT`, Selector, FAQ, module order and all unrelated contracts remain unchanged. V0.13 and V0.14 remain history and are not overwritten.
...
## 11. V0.15 Asset Manifest

| File | Actual dimensions | Bytes | SHA-256 | Status |
|---|---:|---:|---|---|
| `assets/PRODUCT-000_G5_DESKTOP_BUYER_FACING_FINAL_V0.15.png` | 1440×5353 | 392449 | `5F4628FB0F9155A745B32CCF60DCAFCF4480026AEC7A9136A4312D5727B8633E` | `CURRENT_BUYER_FACING_FINAL_CANDIDATE / PROJECT_CONTROL_REVIEW_PASS` |
| `assets/PRODUCT-000_G5_MOBILE_390_BUYER_FACING_FINAL_V0.15.png` | 780×12867; 390px logical at 2× | 634563 | `D17B6AF0D49007BDFB89D48BD583151CC248AFCB0B182423881F82E71AD93B62` | `CURRENT_BUYER_FACING_FINAL_CANDIDATE / PROJECT_CONTROL_REVIEW_PASS` |

Both assets were reviewed at original detail and in cropped original-pixel views. Desktop displays all fourteen exact descriptions in three-zone rows; long M-350, M-510, M-996, M-2196 and M-2377 descriptions wrap within their row without colliding with grade or CTA. Mobile displays grade/action first and the complete description below. No horizontal overflow, crop, overlap or CTA blank band was introduced. V0.13 Hero/geometry and the compact five-step Evaluation treatment remain visible. Buyer-visible Footer labels remain `CONVERSION = 0` and `PROCUREMENT = 1` in each asset.
...
- V0.14 is the immediate visual source. V0.15 changes only the fourteen directory rows, their content-driven group heights and the downstream vertical placement required by the expanded rows.
...
- V0.9 historical assets, V0.10 internal proof, V0.11, V0.12, the project-control-approved V0.13 and the superseded V0.14 draft remain unchanged and are not overwritten.
