# PRODUCT-PROC-CL Gate 2 Content and Operation Contract V0.4
...
| Version / date | V0.4 / 2026-09-06 |
| Status | `REVISED_FOR_PC_CL_01_BUYER_REVIEW` |
| Buyer-copy source | `PRODUCT-PROC-CL_GATE2_CONTENT_SKELETON_V0.4.md` §3 |
| Previous contracts | V0.1–V0.3; preserved unchanged with their reviewed Skeletons |
| Revision basis | `PC-CL-01 = OPEN / REQUIRED_REVISION` in Project Control Review `PRODUCT-PROC-CL-G2-SKELETON-PC-01` |
| Full Buyer Clean Copy | Not created in this phase |

This contract defines what the revised Skeleton's words and actions mean. V0.4 changes only the CL-01, CL-03 and CL-04 application-scope wording required by `PC-CL-01`; it does not create layout, code, CMS fields, receiver behavior, route readiness, publication or indexing authority.
...
| Five range-level application areas named in CL-01 | PRODUCT V0.3 Application sets: Coatings, Plastics, Masterbatch, Printing Inks and Paper each contain at least one current Chloride Grade | Inclusive range context only; does not state or imply that every Grade serves every named application |
| Continuous eight-Grade list and natural overlap guidance | PRODUCT V0.3 Application relationships and the approved Product Hub application-result sets | Direct buyers to inspect the complete list and Grade evidence; no partial per-Grade application matrix, recommendation, ranking or formulation-suitability claim |
| Illustrative evaluation context in CL-04 | Gate 1 approved Application path plus current approved Application categories | Examples help buyers describe a system; they do not establish Grade suitability or take over Application-page education |
...
- do not expand CL-04 into generic Application landing-page content; and
...
There is no new enterprise-fact decision required for this Skeleton. Full Copy, exact transition copy and any later media remain outside this phase. An independent Buyer Review role must verify `PC-CL-01` against Skeleton V0.4 before the package returns to Project Control or user confirmation.
...
## 10. V0.3 second narrow revision record

| Changed location | V0.2 | V0.3 | Contract effect |
|---|---|---|---|
| CL-03 intro application relationship sentence | Named partial M-350 and M-510 application sets, which could be read as exhaustive and omitted M-350 → Paper | Uses the general statement `Application relationships can overlap and are not exhaustively listed here.` | Removes the partial enumeration and expressly prevents exhaustive interpretation without adding a longer application list |
| CL-03 list treatment | One continuous ordered list with both application-labelled subheadings removed | Unchanged | Retains the resolved non-mutually-exclusive structure |
| All Grade entries and other content/actions | V0.2 baseline | Unchanged | No Grade, order, exact summary, URL, CTA, fact, receiver, metadata or Schema change |

## 11. V0.4 Project-Control revision record

| Changed location | V0.3 | V0.4 | Contract effect |
|---|---|---|---|
| CL-01 Hero scope | Named Coatings, Plastics and Masterbatch only | Names Coatings, Plastics, Masterbatch, Printing Inks and Paper as included applications | Lets all five approved Application audiences recognize the range-level path without assigning any application to every Grade |
| CL-03 overlap guidance | Used `Application relationships` and `not exhaustively listed` plus a defensive ranking/suitability sentence | Uses `Because application needs can overlap` and sends buyers directly to the full list and Grade evidence; removes the defensive sentence | Converts an internal relationship rule into buyer guidance without a detailed matrix or suitability claim |
| CL-04 starting context | Gave only Coatings and Plastics/Masterbatch examples | Introduces all details with `For example` and adds concise Printing Inks and Paper context | Makes the examples visibly illustrative and avoids excluding an approved Application audience |
| All Grade entries, modules and actions | V0.3 baseline | Unchanged | No Grade, order, exact summary, URL, CTA, Process claim, receiver, metadata or Schema change |
