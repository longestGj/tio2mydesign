...
W5交接按准备合同V1.5§10：输入W4批准范围、可复用来源、实际容量/限制；交回批次与复用、输入依赖、当前批次准入材料及readiness_scope。总控核对准入和启动权限，W5不派发或维护权威状态；未决owner与关键冲突交总控定向处理。
