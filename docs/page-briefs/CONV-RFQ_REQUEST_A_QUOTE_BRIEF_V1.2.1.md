# Request a Quote Page Brief V1.2.1
...
| User decision source | Directed Gate 1 revision dated 2026-09-01; root `AGENTS.md` §2.4 |
...
V1.2.1 is a targeted Gate 1 correction. V1.2 is preserved as historical and does not control the current page. The supplied design-spec file remains change-input evidence, not a visual baseline.
...
| Gate 1 | `DRAFT_FOR_PROJECT_CONTROL_REVIEW` | This Brief, Gate 1 audit, impact analysis and Manifest; project-control review and user confirmation pending |
...
All V0.x Gate 2–5 artifacts and V1.2 Gate 1 files are retained as history. Submission is not approval and this Brief does not close Gate 1.
...
Checkpoint remains `CONTENT_INTENT_CONFIRMED_PENDING_USER_CONFIRMATION`.
...
The complete page target follows one continuous vertical reading order:

1. Shared Global Header.
2. Standard Page Hero.
3. One vertically ordered RFQ form.
...
Desktop at 1440px must not use left/right page composition, side rails or two-column field pairs. Tablet at 768px and Mobile at 390px retain the same single-column field order. The future Gate 5 package must show the complete final experience even when parallel routes are still being implemented; route functionality is verified at Gate 7–9 and blocks release if unavailable.
...
This V1.2.1 Brief is `DRAFT_FOR_PROJECT_CONTROL_REVIEW`. It creates no visual baseline, does not close `CONTENT_INTENT_CONFIRMED`, does not authorize Gate 2, and does not reactivate historical Gate 2–5 artifacts.
