...
with sync_playwright() as p:
    ...
    results = {
        "desktop": capture(page, 1440, "M350_D32_G3_FULL_1440.png"),
        "tablet": capture(page, 768, "M350_D32_G3_FULL_768.png"),
        "mobile": capture(page, 390, "M350_D32_G3_FULL_390.png"),
    }
    ...
    page.screenshot(path=str(ROOT / "M350_D32_G3_MENU_390.png"), full_page=False)
    ...
    page.screenshot(path=str(ROOT / "M350_D32_G3_COOKIE_390.png"), full_page=False)
...
assets = {}
for path in sorted(ROOT.glob("M350_D32_G3_*.png")):
    assets[path.name] = {"bytes": path.stat().st_size, "sha256": sha256(path)}
results["assets"] = assets
results["source"] = {"bytes": HTML.stat().st_size, "sha256": sha256(HTML)}
(ROOT / "gate3_measurements.json").write_text(json.dumps(results, indent=2), encoding="utf-8")
...
if failures:
    raise SystemExit("\n".join(failures))
print(json.dumps({"status": "PASS_FOR_FORMAL_RENDER", "measurements": str(ROOT / 'gate3_measurements.json')}, indent=2))
