...
const outputPath = path.join(__dirname, 'runtime_observations_home_visual_bundle_v1_1.json');
...
(async () => {
    ...
    for (const width of widths) {
      ...
      const data = await page.evaluate(({ expectedModules, expectedCopy, expectedHrefs }) => {
        ...
        return {
          ...
          chrome: {
            headerHeight: rect(document.querySelector('.global-header')).height,
            desktopNavVisible: visible(document.querySelector('.desktop-nav')),
            desktopCurrentCount: [...document.querySelectorAll('.desktop-nav [aria-current="page"]')].filter(visible).length,
            mobileBarVisible: visible(document.querySelector('.mobile-bar')),
            mobileLabels: [...document.querySelectorAll('.mobile-bar > :is(a,button)')].filter(visible).map((element) => (element.textContent || element.getAttribute('aria-label') || '').trim().replace(/\s+/g, ' ')),
            mobileRfqVisibleText: [...document.querySelectorAll('.mobile-rfq > span')].filter(visible).map((element) => element.textContent.trim()).join(' '),
            legal: [...document.querySelectorAll('.legal-utilities > *')].map((element) => ({ tag: element.tagName.toLowerCase(), label: element.textContent.trim(), href: element.getAttribute('href'), rect: rect(element) })),
            footerRfq: rect(document.querySelector('.footer-rfq')),
          },
      ...
      add(`viewport.${width}.header-geometry`, data.chrome.headerHeight === (width > 1100 ? 84 : 64), JSON.stringify(data.chrome));
      add(`viewport.${width}.header-surface`, width > 1100 ? (data.chrome.desktopNavVisible && data.chrome.desktopCurrentCount === 1 && !data.chrome.mobileBarVisible) : (!data.chrome.desktopNavVisible && data.chrome.desktopCurrentCount === 0 && data.chrome.mobileBarVisible), JSON.stringify(data.chrome));
      add(`viewport.${width}.compact-rfq`, width > 1100 || data.chrome.mobileRfqVisibleText === 'RFQ', JSON.stringify({ labels: data.chrome.mobileLabels, visibleText: data.chrome.mobileRfqVisibleText }));
      add(`viewport.${width}.legal-order`, JSON.stringify(data.chrome.legal.map(({ tag, label, href }) => ({ tag, label, href }))) === JSON.stringify([
        { tag: 'a', label: 'Privacy Policy', href: '/privacy-policy/' },
        { tag: 'a', label: 'Dasar Privasi (BM)', href: '/ms/privacy-policy/' },
        { tag: 'a', label: 'Cookie Policy', href: '/cookie-policy/' },
        { tag: 'button', label: 'Cookie Settings', href: null },
      ]), JSON.stringify(data.chrome.legal));
      add(`viewport.${width}.footer-targets`, data.chrome.legal.every(({ rect }) => rect.width >= 44 && rect.height >= 44) && data.chrome.footerRfq.width >= 44 && data.chrome.footerRfq.height >= 48, JSON.stringify({ legal: data.chrome.legal, footerRfq: data.chrome.footerRfq }));
      ...
      if (state === 'menu') {
        const menu = await page.evaluate(() => {
          ...
          return {
            ...
            background: getComputedStyle(element.querySelector('nav')).backgroundColor,
            current: current?.textContent.trim() ?? null,
            currentMarkerWidth: current ? getComputedStyle(current, '::before').width : null,
            terminal: terminal?.textContent.trim() ?? null,
            terminalBackground: terminal ? getComputedStyle(terminal).backgroundColor : null,
            topbarLabels: [...element.querySelectorAll('.menu-topbar > :is(a,button)')].map((node) => (node.textContent || node.getAttribute('aria-label') || '').trim().replace(/\s+/g, ' ')),
            visibleCurrentLabels: [...element.querySelectorAll('*')].filter((node) => node.children.length === 0 && node.textContent.trim() === 'CURRENT').length,
          };
        });
        ...
        add('state.menu.order', JSON.stringify(menu.labels) === JSON.stringify(['Home', 'Markets', 'Products', 'Applications', 'Documents', 'Resources', 'About', 'Request a Quote']), JSON.stringify(menu.labels));
        add('state.menu.shared-treatment', menu.background === 'rgb(3, 27, 58)' && menu.current === 'Home' && menu.currentMarkerWidth === '4px' && menu.terminal === 'Request a Quote' && menu.terminalBackground === 'rgb(0, 128, 120)', JSON.stringify(menu));
        add('state.menu.topbar', JSON.stringify(menu.topbarLabels) === JSON.stringify(['TiO2 Malaysia home', 'RFQ', 'Close']), JSON.stringify(menu.topbarLabels));
        add('state.menu.no-current-label', menu.visibleCurrentLabels === 0, `count=${menu.visibleCurrentLabels}`);
      } else {
  ...
  const output = {
    verifier: 'HOME-001-APPLICATIONS-ALIGNED-RUNTIME-V1.1',
