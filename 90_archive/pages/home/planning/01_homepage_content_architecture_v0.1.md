...
以下顺序对应推荐的“方案 1：市场优先的采购路径型首页”。所有标题均为工作标题，不是最终营销文案。
...
| 2 | `[Hero：Malaysia TiO₂ 采购入口]` | 在首屏建立首页主题、受众和第一行动 | 这是什么供应平台？是否服务工业采购？下一步是什么？ | `malaysia titanium dioxide`；supplier 语义 | 1 个 H1 主题；2–3 句说明性占位；主要受众/范围；不展开 origin 证明 | RFQ、Products | Request a Quote | View Products | 品牌关系、可公开供应定位、受众范围 | 只写中性平台定位；不写 manufacturer、stable supply、lead time 或证书承诺 |
| 3 | `[快速采购路径]` | 让买家立即选择 Market、Product 或 Documents 路径 | 我应该按目的市场、牌号还是文件开始？ | supplier / product selection / documents 语义 | 三个紧凑入口及一句说明；不使用虚构数据 | Markets、Products、Documents | Explore Markets | View Products / Documents | 页面架构已批准 | 不需要企业事实；只陈述页面功能 |
...
| 5 | `[Products：14 Grades 与选型路径]` | 明确产品深度，并把买家导向 Hub、分组、工艺和型号页 | 有多少型号？我如何按应用或工艺选型？ | `titanium dioxide pigment`、rutile、grades、chloride/sulfate 概述 | 明示 `14 grades`；四个展示分组；桌面端紧凑显示 14 个型号 ID；Chloride/Sulfate 入口；选型说明 | Products Hub、两个工艺页、14 型号页 | View All 14 Grades | Chloride Process / Sulfate Process | 产品清单和分类；各型号技术资料 | 只显示已批准型号和分类；M-2377 不标工艺；技术性能未核验则不显示 |
| 6 | `[Applications：按使用场景进入]` | 让技术/采购人员从应用进入，不与型号页争词 | 哪些页面帮助我按应用匹配 TiO₂？ | coatings、plastics、masterbatch、printing inks、paper | 五个应用入口；每项只描述买家决策主题；明确 TiO₂ for masterbatch 不是 finished masterbatch | Applications Hub 与五个应用页 | Explore Applications | View Suitable Grades | 应用架构；最终型号映射和技术资料 | 子页 URL 仍为 provisional 时不锁死最终链接；不发布未核验推荐 |
...
| 8 | `[Documents：文件类型与申请]` | 把文件需求导向 Documents 与受控申请流程 | 我可以了解或申请哪些类型的技术、质量、原产地文件？ | TDS、SDS、COA、COO、REACH-related 语义概述 | 文件类别、用途、适用范围说明；“是否可提供”由状态决定；申请资格提示 | Documents Hub、REACH、TDS/SDS/COA、COO、Request Documents | Request Documents | View Document Information | 文件目录、版本、适用型号/市场、批准用语、申请流程 | 未核验文件显示为“需要确认”或不渲染；不得写 available、compliant 或 covered |
...
| 10 | `[Final RFQ：提交采购需求]` | 将已获得上下文的访客转为有效询盘 | 如何提交目的地、应用、型号/指标、数量、包装、文件和样品需求？ | quote / purchase action；不抢其他页主词 | RFQ 价值说明；字段摘要；隐私/响应占位；样品仅为条件项 | `/request-a-quote/`；条件性 Sample | Request a Quote | Request a Sample（仅流程批准后） | RFQ 负责人、字段、隐私同意、真实响应规则、Sample 资格 | 没有 SLA 不写响应时间；样品流程未批准则不显示 Sample CTA |
...
桌面端可在四个分组内紧凑列出全部 14 个型号 ID。移动端首层显示四组及数量，并提供明确的 `View All 14 Grades`；组内型号可通过展开面板查看，但不能用 14 段长正文拖长首页。
...
| 应用 | 子页主关键词 | 首页处理 | 与型号词的边界 |
|---|---|---|---|
| Coatings | `titanium dioxide for coatings` | 第一优先应用入口；可同时解释 paint 语义归并 | 只导向应用页和 verified grades，不替型号页拥有型号词 |
| Plastics | `titanium dioxide for plastics` | 第一优先应用入口 | 不在首页发布 PVC/PE/PP/ABS/PET 的完整技术正文 |
| Masterbatch | `titanium dioxide for masterbatch` | 第一优先应用入口；明确是 TiO₂ input | 不把产品写成 finished titanium dioxide masterbatch |
| Printing Inks | `titanium dioxide for printing inks` | 次一级应用入口 | M-52 等型号仍由独立型号页拥有型号词 |
| Paper | `titanium dioxide for paper` | 较低优先级入口，但不得缺失 | 只展示入口和买家问题，不虚构适用型号 |

Applications Hub 和五个子页的最终 URL/映射仍需后续 Applications 设计批准。首页架构可以预留入口，但不能把 provisional URL 当作最终路由锁定。
...
- 首页次级转化。
- 主要出现在 Documents 模块；Hero 是否出现文件 CTA 需主控审查，不作为默认强制项。
- 信息页解释文件；`/request-documents/` 只负责受控申请和线索采集，不拥有信息型关键词。
- 在版本、适用型号、市场和审批条件未核实前，不宣称文件 available。

### I.3 Sample

- 首页第三级、条件性转化，不进入 Header 主导航，也不与 RFQ 等权。
- 优先从 Product、Application 或 Market 页面带上下文进入 `/request-sample/`。
- 首页最多在 Final RFQ 作为文字级次 CTA；样品资格、数量、运费和交付限制未批准时完全不显示。
...
| 桌面模块 | 移动端处理 |
|---|---|
| Hero + 快速采购路径 | 合并为首屏和紧随其后的 3 个文本入口，避免复杂悬浮面板 |
...
| Final RFQ | 单列；RFQ 主按钮全宽；Sample 仅条件显示 |

移动端目标不是机械缩小桌面页，而是保留：网站是什么、服务哪个市场、有哪些产品路径、如何核实、如何询价。删除装饰性重复内容，避免页面过长。
...
说明：战略定位“Malaysia-Origin”不等于所有制造、COO、批次或具体交易事实已经核验。正式 origin 声明仍需证据登记。
...
推荐方案 1。它最符合 PRD V0.4 的市场优先原则，同时通过“快速采购路径”和紧随 Markets 的 Products 模块避免产品被弱化。视觉设计不得改变该内容顺序，除非主控重新批准架构版本。
...
- [x] RFQ、Documents、Sample 的主次与跳转职责已分开。
...
- [ ] 批准方案 1 作为首页正式内容顺序。
- [ ] 确认 Hero 次 CTA 使用 `View Products`，而不是默认 `Request Documents`。
- [ ] 确认桌面 Products 模块是否紧凑显示全部 14 个型号 ID。
- [ ] 确认首页是否在 Final RFQ 保留条件性的 Sample 文字入口。
- [ ] 确认 Applications provisional URLs 在视觉/开发前是否需要先升级批准。
- [ ] 确认 Documents 子页与 Request Documents 流程何时进入下一版 PRD。
- [ ] 审核事实证据清单，明确哪些企业资料可以转为 `verified`。
- [ ] 主控完成内容架构审查后，才进入下一阶段；本文件本身不授权视觉或开发。
