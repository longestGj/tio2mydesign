...
按[准备合同](../../../docs/architecture/WEBSITE_PREPARATION_HANDOFF_CONTRACT_V1.6.md)以site_id和preparation_scope恢复原实例。先复用当前来源，定位W1–5差额；接收专业结果与独立链，再记录用户决定和对应接收包/阶段的就绪范围。网站基线只保存引用。无授权不启动页面；新增页面不重跑整站准备；并行工作继续按原授权和实际槽位执行。

W5计划由总控结合当前执行/审查容量及共享依赖采用，原控制记录保存实际安排；只改受影响波次，不新建全站任务包。W5不承担运行派发和关闭，生产批次不等于发布范围。

批次准备触发：下一批需要准入、代表成果可复用、关键输入变化。总控先核对现有输入及准备授权，只指派W5补受影响批次，已有足够输入不重复制作。接收适用标准的来源/建立任务、owner、消费阶段和批准依据；消费前核对完成情况。早期工作包就绪不要求Gate 0材料，W5就绪与页面启动分别记录。
