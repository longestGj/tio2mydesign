...
if(fs.existsSync(path.join(dir,'freeze.json'))&&!process.argv.includes('--replace-unapproved-r1'))throw Error('existing freeze: create new candidate rather than overwrite');
if(process.argv.includes('--replace-unapproved-r1')&&!fs.existsSync(path.join(dir,'diagnostic_support/superseded-freeze-r1/freeze.json')))throw Error('replacement requires preserved prior freeze');
