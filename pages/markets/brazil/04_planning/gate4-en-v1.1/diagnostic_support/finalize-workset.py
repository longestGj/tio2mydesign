...
inputs = [
 ...
 (project/"pages/markets/brazil/MARKET-BR-EN_CURRENT_GATE_BASELINE_MANIFEST_V0.6.md", "V1.0 candidate Manifest under revision"),
 (project/"pages/markets/brazil/05_review/MARKET-BR-EN_GATE4_INDEPENDENT_REVIEW_V1.0/report.md", "independent first-review findings"),
 (project/"pages/markets/brazil/05_review/MARKET-BR-EN_GATE4_INDEPENDENT_REVIEW_V1.0/observations.json", "independent first-review observations"),
...
input_path = gate/"MARKET-BR-EN_GATE4_INPUT_INDEX_V1.1.json"
input_path.write_text(json.dumps({"worksetId":"BR-EN-G4-WS-02","pageId":"MARKET-BR-EN","items":[identity(p,r) for p,r in inputs]},indent=2),encoding="utf-8")
...
dep_path=gate/"MARKET-BR-EN_GATE4_DEPENDENCY_INDEX_V1.1.json"
dep_path.write_text(json.dumps({"worksetId":"BR-EN-G4-WS-02","items":[identity(p,r) for p,r in deps]},indent=2),encoding="utf-8")

source=gate/"MARKET-BR-EN_GATE4_COMPLETE_VISUAL_V1.1.html"
inventory=gate/"approval_core/export-inventory.json"
freeze_path=gate/"MARKET-BR-EN_GATE4_FREEZE_RECORD_V1.1.json"
freeze={"worksetId":"BR-EN-G4-WS-02","freezeId":"BR-EN-G4-V11-SOURCE-01","status":"COMPLETE_VISUAL_FROZEN / DRAFT_FOR_INDEPENDENT_REREVIEW","frozenAt":datetime.now(timezone.utc).isoformat(),"source":identity(source,"single editable visual source"),"inputIndex":identity(input_path,"exact frozen inputs and original findings"),"dependencyIndex":identity(dep_path,"exact durable dependencies"),"formalInventory":identity(inventory,"formal export inventory"),"render":{"browser":"Chrome 152.0.7977.82","viewports":["1440x900 DPR1","768x900 DPR1","390x844 DPR2 (780 physical width)"],"states":["normal full page","document action keyboard focus at all viewports","menu open where applicable","Cookie settings open"]}}
freeze_path.write_text(json.dumps(freeze,indent=2),encoding="utf-8")

evidence_files=[source,gate/"MARKET-BR-EN_GATE4_COMPLETE_VISUAL_AND_METHOD_REPORT_V1.1.md",gate/"MARKET-BR-EN_GATE4_IR01_IR02_REVISION_RESPONSE_V1.1.md",review/"MARKET-BR-EN_GATE4_EXECUTION_SELF_CHECK_V1.1.md",inventory,gate/"diagnostic_support/preflight-runtime.json",gate/"diagnostic_support/formal-runtime-and-export.json",gate/"diagnostic_support/readable-segments/segments.json",freeze_path]
...
evidence_path=gate/"MARKET-BR-EN_GATE4_EVIDENCE_INDEX_V1.1.json"
evidence_path.write_text(json.dumps({"worksetId":"BR-EN-G4-WS-02","freezeId":"BR-EN-G4-V11-SOURCE-01","approvalCoreRoles":["frozen source","three full-page PNGs","eight applicable state PNGs","export inventory","method and revision reports","author self-check","freeze record"],"items":[identity(p,"approval_core" if "approval_core" in p.parts or p in [source,freeze_path,gate/"MARKET-BR-EN_GATE4_COMPLETE_VISUAL_AND_METHOD_REPORT_V1.1.md",gate/"MARKET-BR-EN_GATE4_IR01_IR02_REVISION_RESPONSE_V1.1.md",review/"MARKET-BR-EN_GATE4_EXECUTION_SELF_CHECK_V1.1.md"] else "diagnostic_support") for p in evidence_files]},indent=2),encoding="utf-8")

workset_path=gate/"MARKET-BR-EN_GATE4_WORKSET_V1.1.json"
workset_path.write_text(json.dumps({"worksetId":"BR-EN-G4-WS-02","pageId":"MARKET-BR-EN","scope":"targeted correction for BR-EN-G4-IR-01 and BR-EN-G4-IR-02","status":"COMPLETE_VISUAL_FROZEN / DRAFT_FOR_INDEPENDENT_REREVIEW","visualDirectionCheckpoint":"VISUAL_DIRECTION_CHECKED / unchanged","designSource":identity(source,"single V1.1 design source"),"inputIndex":identity(input_path),"dependencyIndex":identity(dep_path),"freezeRecord":identity(freeze_path),"evidenceIndex":identity(evidence_path),"stop":"independent targeted rereview required; no user approval, Gate 4 closure or Gate 6"},indent=2),encoding="utf-8")
