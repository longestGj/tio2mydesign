# PRODUCT-000 Full Visual Design V0.12 — Desktop Directory Baseline Alignment Revision
...
| Version | `V0.12` |
| Date | `2026-08-31` |
| Review ID | `PRODUCT-G5-BUYER-FINAL-UX-04` |
...
| Output set | Desktop V0.16 at 1440px plus unchanged 390px logical Mobile V0.15 at 2× |
| Scope | Desktop-only vertical baseline repair for single-line directory descriptions |
...
This revision does not redesign the page or change any copy. It follows the user's 2026-08-31 Desktop alignment instruction: a one-line directory description shares the vertical centerline used by the grade name and `View Grade →`, while a two-line description retains the approved V0.15 top position. Mobile V0.15 is unchanged and remains the paired Mobile candidate. Header/Footer, fixed RFQ, `PROCUREMENT`, Selector, Hero, Evaluation, FAQ, relationships, dimensions and all unrelated contracts remain unchanged. V0.15 is not overwritten.
...
- Every directory row has three information zones on Desktop: fixed grade name, neutral application/process description and `View Grade →`. Mobile places grade and action on the first line and the complete description on the second line.
- Desktop measures the rendered description at 326px. A one-line result (`≤25px` measured height) starts at row `y+21`, matching the grade and action baseline; a two-line result (`>25px`) retains row `y+13`.
- The rule is content-responsive and applies to all fourteen rows. It does not use per-grade manual offsets.
- Descriptions are identification summaries, not recommendations, suitability guarantees or comparative selection reasons.
...
|---|---|
| M-350 | `Coatings, plastics, printing inks & paper · Chloride process` |
| M-510 | `Coatings, plastics, masterbatch & printing inks · Chloride process` |
| M-896 | `Coatings · Chloride process` |
| M-996 | `Industrial, powder & architectural coatings · Sulfate process` |
| M-2196 | `Solvent-based furniture & industrial coatings · Sulfate process` |
| M-895 | `Coatings · Chloride process` |
| M-200 | `Plastics & masterbatch · Chloride process` |
| M-108 | `Plastics & masterbatch · Sulfate process` |
| M-210 | `Plastics & masterbatch · Chloride process` |
| M-340 | `Plastics & masterbatch · Chloride process` |
| M-886 | `Plastics & masterbatch · Chloride process` |
| M-52 | `Coatings & printing inks · Sulfate process` |
| M-2377 | `Coatings, plastics, masterbatch, printing inks & paper · Sulfate process` |
...
| Product Directory | Grade name and `View Grade` only; buyer had to open every detail page to understand even the neutral application/process identity | Four existing groups retained; all fourteen rows add the exact A5.1 application/process description; Desktop three-zone and Mobile two-line responsive treatment | User-directed decision clarity, Product relationship, GEO |
| Desktop directory alignment | All descriptions used the two-line top offset, causing one-line descriptions to sit above the grade/action centerline | Measured-height rule: one-line `y+21`; two-line `y+13`; no copy or row-size change | User-directed visual alignment repair |
...
## 11. V0.16 Desktop + V0.15 Mobile Asset Manifest

| File | Actual dimensions | Bytes | SHA-256 | Status |
|---|---:|---:|---|---|
| `assets/PRODUCT-000_G5_DESKTOP_BUYER_FACING_FINAL_V0.16.png` | 1440×5353 | 393290 | `5AB83A4BC503CD2326A622AB8E87AFAA848F332413436C617E869D8DD2325578` | `APPROVED_GATE_5_BASELINE` |
| `assets/PRODUCT-000_G5_MOBILE_390_BUYER_FACING_FINAL_V0.15.png` | 780×12867; 390px logical at 2× | 634563 | `D17B6AF0D49007BDFB89D48BD583151CC248AFCB0B182423881F82E71AD93B62` | `APPROVED_GATE_5_BASELINE` |

Desktop V0.16 was reviewed at original detail and in cropped original-pixel views. Single-line descriptions align with the grade/action centerline; M-350, M-510, M-996, M-2196 and M-2377 retain their two-line placement. No description, grade or CTA collides. Mobile V0.15 is byte-for-byte unchanged. No horizontal overflow, crop, overlap or CTA blank band was introduced. Buyer-visible Footer labels remain `CONVERSION = 0` and `PROCUREMENT = 1` in each asset.
...
- Desktop V0.15 is the immediate visual source. Desktop V0.16 changes only the vertical origin of one-line directory descriptions; dimensions, copy, row geometry and all other pixels remain protected. Mobile V0.15 is reused unchanged.
...
- `PRODUCT-G5-BUYER-FINAL-01 = APPROVED / CLOSED`; user approval recorded 2026-08-31 for the V0.12 + Desktop V0.16 + Mobile V0.15 Gate 5 baseline.
...
- `PRODUCT-G5-BUYER-FINAL-UX-03 = PROJECT_CONTROL_REVIEW_PASS / CLOSED`; V0.11 and both V0.15 assets are the current Buyer-Facing Final candidate baseline pending the parent user decision.
- `PRODUCT-G5-BUYER-FINAL-UX-04 = PROJECT_CONTROL_REVIEW_PASS / CLOSED`; V0.12, Desktop V0.16 and unchanged Mobile V0.15 are the current Buyer-Facing Final candidate baseline pending the parent user decision.
- Gate 5 is approved. The page visual endpoint is `APPROVED_GATE_5_BASELINE`.
- This approval hard-stops at Gate 5. It does not authorize Gate 6/7, Process or Grade child-page work, code, CMS, development handoff, deployment, release or publication.
