...
await page.close();
const result={pageId:'APP-000',author:'/root/app000_gate3_author',createdAt:new Date().toISOString(),renderer:await browser.version(),source:{path:dir+'/index.html',sha256:crypto.createHash('sha256').update(fs.readFileSync(dir+'/index.html')).digest('hex')},checks,status:checks.every(c=>c.pass)?'PASS':'FAIL'};
