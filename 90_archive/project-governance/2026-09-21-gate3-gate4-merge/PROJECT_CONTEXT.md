...
> 当前阶段职责见[Gate工作流V3.3](docs/architecture/GATE_WORKFLOW_V3.3.md)：Gate 3验证结构；Gate 4完成视觉（4A内部检查、4B最终关闭），原Gate 5并入4；Gate 6总审并交付，原Gate 7并入6；Gate 8由D32负责技术承接与实现，当前已批准首页采用WordPress直接管理内容并生成页面，Gate 9独立只读验收，Gate 10另行发布授权。以下项目事实与页面状态不因此改变。
...
共享Header/Footer由Home/Global Chrome owner维护，页面消费，不各自开发；前台不展示“CURRENT”文字。开发由 `D:\32Wordpress_new` 独立项目承接，当前首页采用WordPress直接管理内容并生成页面；继续兑现 `site_scope=tio2-my` 的站点隔离要求。旧WordPress/Next.js实现不自动成为D32技术基线。
