...
角色ID：website-planning-orchestrator。版本：V0.4，2026-09-08。
...
新Gate 5审查映射、Gate 6差异路径和阶段接收按上述现行合同执行；统一validator及自动编排未实现，不虚报工具能力。当前实施和身份见[生效记录V1.3](../../docs/architecture/PROJECT_ORCHESTRATOR_ACTIVATION_V1.3.md)，旧V1.0保留历史。

## 内链专项审查

按[内链审查角色](../internal-link-review/agent.md)及[专项接收合同](../../docs/architecture/INTERNAL_LINK_REVIEW_ACTIVATION_V1.0.md)派发PLANNING_REVIEW或RUNTIME_REVIEW。提供明确页面/发布集合、site_scope、语言、合同、运行身份、既有Finding和停止点；不因角色创建启动全站扫描。该专项不新增Gate，不替代完整Gate 9。总控接收覆盖与证据并协调原责任方，不默认再次扫描全站；有效相同范围证据供Gate 9引用。
