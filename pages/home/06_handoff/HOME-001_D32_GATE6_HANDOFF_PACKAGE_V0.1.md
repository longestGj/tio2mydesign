# HOME-001 + Global Chrome：D32 WordPress Gate 6 交付包 V0.1
...
|---|---|
| 日期 | 2026-09-20 |
| Package / Handoff ID | `HOME-001-D32-G6-PACKAGE-V0.1` / `HOME-001-D32-G6-HANDOFF-01` |
...
路径相对本文件；每个实际输入的大小和 SHA-256 见 [输入与自检证据](HOME-001_D32_GATE6_INPUT_VERIFICATION_V0.1.json)。哈希用于本次交接身份，不进入网站内容读取条件。
...
| Products | 标题/说明、四组标题/说明、型号显示名与目标、工艺入口 | 初始四组 6/5/2/1、14 型号完整唯一；首页清单不自动产生14详情页或产品集合页 |
...
**本候选建议继承用户已批准的五节点完整图谱**，按专属合同 §2/§5 的具体属性与引用核验，不用“关系数量”简写替代实际边。此为显式冲突处置提议 `HOME-D32-G6-F01`，须由后续独立复核/总控确认后才关闭；本轮不自行删除 Product、不改CSV、不声称冲突已正式关闭。
...
| A05 | 四组14型号完整唯一，顺序/工艺表达保留；移动默认折叠并能访问全部型号 | 数据/DOM清单、键盘展开收起、aria-expanded和390展开图；重复/漏项/隐藏后不可达或推导推荐失败 |
...
| HOME-D32-G6-F01 | Schema总表与专属合同差异；D23总控/独立复核处理§5.2提议 | 明确接受继承五节点或提交相反来源；未正式关闭不能将整个Gate6标APPROVED_FOR_HANDOFF |
...
后续需同步的唯一入口由总控在关闭时处理：首页当前Manifest与Status相关行、D32派发/回执；如§7需要变更共用接口则定向升级其合同/工具，不批量改全站AGENTS或其他页面状态。本轮不执行这些关闭/派发动作。
...
没有运行浏览器、WordPress、构建或开发测试；没有重做独立视觉审查。F01已显式登记，独立复核和Gate6关闭未执行。本文档编制完成不等于开发就绪批准。
