...
总控编排当前入口：[角色V0.6](agents/project-orchestrator/agent.md)及[生效与身份V1.5](docs/architecture/PROJECT_ORCHESTRATOR_ACTIVATION_V1.5.md)。用于状态恢复、准入、派发、接收、异常及关闭；按当前交接合同派发新Gate 5审查与Gate 6差异路径。
...
| Gate 9 Agent与方法改进 | [当前基线Manifest V1.0](docs/architecture/GATE9_AGENT_SKILL_CURRENT_BASELINE_MANIFEST_V1.0.md)：[角色V0.5](agents/gate9-read-only-acceptance/agent.md)＋[运行方法V0.4](skills/runtime-implementation-verification/SKILL.md)＋两个只读预检脚本＋[交接合同V1.0](docs/architecture/GATE8_GATE9_EVIDENCE_HANDOFF_CONTRACT_V1.0.md)＋[D16本地main队列规则](docs/architecture/D16_LOCAL_MAIN_INTEGRATION_QUEUE_RULE_V1.0.md)。D16已在commit `3ec98f34a8ee8dbb4cdca1ff7be6c5f25c32260b`采用，见[独立读回](docs/architecture/D16_GATE9_INTERFACE_ADOPTION_RECEIPT_V1.0.md)。当前组合实现机器预检、四层状态、共享根Finding、固定收口及Gate8通知；7/7工具测试通过，待下一批真实页面试跑。项目源未运行注册；`INTEGRATION_READY`不自动授权合并或发布。 |
