# About TiO2 Malaysia Page Brief V0.1
...
| 总控审核人 | 项目总控与质量审查 |
| 用户批准记录 | 当前来源任务要求先完成 ABOUT-001 Gate 0/1 合规审计并提交；尚未批准 Gate 0、Gate 1 或 Gate 2–5 成果 |
...
| 本版变更摘要 | 建立页面身份、SEO/GEO 边界、事实与证据门禁、Global Chrome 继承、后续状态覆盖要求和 Gate 0/1 审查合同；未进入 Gate 2–5 |
...
| 11 | 本页 Gate 0/1 审计 | `pages/about-contact/01_research/ABOUT-001_GATE0_GATE1_COMPLIANCE_EVIDENCE_AUDIT_V0.1.md` | 公司、制造、origin、质量、追溯、出口、Schema 和渲染门禁 | `SUBMITTED_FOR_GATE_0_GATE_1_REVIEW` |
| 12 | 当前一方公开站点 | `mytio2.com` 的 Home/About/Documents/Contact；2026-08-30 抓取复核 | 主体、地址、制造、产能、包装、文件和出口声明的候选交叉核对 | `CURRENT_FIRST_PARTY_CANDIDATE_ONLY`；不得复制正文或自动发布 |
...
| `ABOUT-R002` | 项目治理把运营主体名称列为当前基线，但没有 SSM 公司档案、注册号、状态、注册地址和业务性质证据 | Organization/公司事实深度 | 可按项目批准基线显示完整名称和品牌—运营关系；不得扩写为“注册制造商”、经营状态、注册地址或法定事实 | 提供当前 SSM Company Profile / ACGS 或等效批准文件 |
| `ABOUT-R003` | 一方站点声明 Taiping 制造、Malaysia origin、35,000 t/y、30+ 国家、200+ 客户、包装、文件与 Port Klang 出口，但本项目无原始证明材料 | 本页核心信任叙事 | 全部登记为候选；逐字段 `DO_NOT_RENDER`，不得用视觉暗示替代证据 | 提供制造/原产地/产能/客户/包装/文件/出口的一方原始证据并经审核 |
| `ABOUT-R004` | 地址只在一方站点重复出现，地址类型未确认 | 地址、地图、Organization Schema | `DO_NOT_RENDER`；不称为 factory、registered office、office 或 visitor address | SSM/租赁或产权/许可/用户批准文件明确地址及类型 |
| `ABOUT-R005` | 无已批准工厂、生产线、仓库、装柜、证书或团队图像及授权记录 | Gate 3–5 视觉 | 使用无工厂图片状态；禁止生成或使用暗示制造能力的替代图片、工厂图标或证书徽章 | 资产来源、拍摄地、对象、日期、授权和事实对应关系获批 |
...
- 运营主体完整名称可按项目批准基线进入可见文本；`legalName` 是否进入 Schema 需 Gate 2 结合 SSM 证据决定。注册号、公司状态、注册地址和业务性质当前全部不渲染。
- manufacturer、manufacturing location、country of origin、capacity、packaging、quality systems、certifications、batch traceability、export countries、customer count 与 Port Klang 关系不得由关键词、图片、图标、ALT、Meta、OG、Schema 或时间线推断生成。
- Answer-ready Blocks 必须携带来源、资料日期、适用范围和验证状态；来源不足时显示中性“evidence pending”解释，或整个事实单元省略，不显示假 badge。

## 5. Gate 1 事实与证据门禁

完整审计见 `pages/about-contact/01_research/ABOUT-001_GATE0_GATE1_COMPLIANCE_EVIDENCE_AUDIT_V0.1.md`。
...
| SSM 注册号、状态、注册地址、业务性质 | `NO_OFFICIAL_PROFILE_OBTAINED` | `DO_NOT_RENDER` | 无注册事实状态 |
| Taiping 地址/制造地点 | `FIRST_PARTY_CANDIDATE_ONLY` | `DO_NOT_RENDER` | 无工厂图片、无地点状态 |
| Malaysia manufacturing / made in Malaysia | `FACT_EVIDENCE_REQUIRED` | `DO_NOT_RENDER` | 受限状态与证据请求路径 |
| Malaysia country of origin / COO | `FACT_EVIDENCE_REQUIRED` | `DO_NOT_RENDER`；只可导向 Documents owner，且不承诺可用 | Documents 关系设计 |
| 35,000 t/y、30+ 国家、200+ 客户 | `UNVERIFIED_SCALE_CLAIMS` | `DO_NOT_RENDER` | 无数字状态 |
| 25kg / pallet / big bag | `UNVERIFIED_PACKAGING_CLAIM` | `DO_NOT_RENDER` | 不依赖规格的采购路径 |
| 质量控制/质量体系 | `NO_APPROVED_QMS_EVIDENCE` | `DO_NOT_RENDER` | 中性质量证据框架 |
| ISO/REACH/FDA 等认证或合规 badge | `NO_APPROVED_COMPANY_CERTIFICATION_EVIDENCE` | `DO_NOT_RENDER` | 无证书状态；REACH 等移交 Documents owner |
| 批次追溯 | `NO_APPROVED_TRACEABILITY_ARTIFACT` | `DO_NOT_RENDER` | 中性流程框架、证据槽位 |
| Port Klang / FCL / LCL /出口市场 | `FIRST_PARTY_OR_ENTITY_MISMATCH_CANDIDATE_ONLY` | `DO_NOT_RENDER` | 无港口/集装箱图片状态 |
| 工厂/生产线/仓库/证书图片 | `NO_VERIFIED_ASSET` | `DO_NOT_RENDER` | 文字与证据卡布局，不留空图框 |

## 6. 后续 Gate 覆盖要求（当前未进入）

| Gate | 本任务卡当前记录 | 获准后必须覆盖 |
|---|---|---|
| Gate 2 | `NOT_STARTED / NOT_AUTHORIZED` | 真实证据分级叙事、H1/Title/Meta、Answer-ready Blocks、来源/更新时间、CTA/内链、Organization Schema 可见一致性；不得把候选事实写成正文 |
...
|---|---|---|---|---|---|
| Gate 0 | ABOUT-001 Brief V0.1 | 2026-08-30 | `SUBMITTED_FOR_GATE_0_REVIEW` | `PENDING` | 页面身份与边界一致；Playbook 缺失阻塞正式 Gate 2 |
| Gate 1 | Gate 0/1 合规与证据审计 V0.1 | 2026-08-30 | `SUBMITTED_FOR_GATE_1_REVIEW` | `PENDING` | 仅品牌与限定运营关系可渲染；核心事实冻结 |
...
2. 是否接受 Gate 1 的字段级冻结与“项目批准运营关系可限定呈现、法定公司事实仍待 SSM”的分级。
3. 是否先建立并批准 `ABOUT_CONTACT_PLAYBOOK_V0.1.md`，或为 ABOUT-001 明确批准页面专属准入例外。
4. 是否授权 Gate 2–5 仅在冻结字段不渲染、无工厂图片和三种证据状态覆盖的前提下继续。
5. 谁负责提供并批准 SSM、地址类型、制造/原产地、产能、包装、质量、认证、追溯、出口和图片授权证据。
