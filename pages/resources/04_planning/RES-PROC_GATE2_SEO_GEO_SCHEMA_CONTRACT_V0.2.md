# RES-PROC Gate 2 SEO / GEO / Schema Contract V0.2
...
| Content authority | `RES-PROC_GATE2_CONTENT_ARCHITECTURE_V0.2.md` |
| Previous version | `RES-PROC_GATE2_SEO_GEO_SCHEMA_CONTRACT_V0.1.md`; retained as pre-normalization history |
...
Use the exact three-sentence direct answer from the content architecture. It must include possible route influence, label-only insufficiency, grade-specific evidence and, where relevant, application-specific testing under the buyer's intended conditions.
...
Direction: route can indicate formation context; the label alone cannot establish grade-level optical, colour, durability, dispersion, processing, regulatory or equivalence conclusions. It also does not establish price, production economics, availability or general versatility, which depend on the specific product, application, supplier and transaction.
...
Direction: LB Group's named chloride- and sulfate-process grades directly establish plastics-family overlap. Tronox supplies broader portfolio-existence context only: it states that it uses both processes to make products used by manufacturers of coatings, plastics and décor paper. Neither source establishes identical performance or substitution, and Tronox is not used to claim that each route separately covers all three applications.
...
- LB Group manufacturer sources support direct named-grade plastics overlap; Tronox supports broader portfolio-existence context only and cannot prove that each route separately covers every listed application.
...
- Maximum content review interval: 180 days after publication, inherited from the approved Buying Guide rule in `RESOURCE_PLAYBOOK_V0.1.md` Section 6.2; this page does not create a new SLA.
...
`RES-PROC-G2-SKELETON-PCR-02 = PROJECT_CONTROL_REVIEW_PASS_PENDING_USER_APPROVAL`
