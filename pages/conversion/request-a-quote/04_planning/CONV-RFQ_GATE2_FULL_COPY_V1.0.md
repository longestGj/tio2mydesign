# CONV-RFQ Gate 2 Full Copy V1.0
...
- Empty option: `Select a destination country`
- Values: the approved country list used by the shared form system; no region label satisfies this required field.
- Selection does not state serviceability, freight, lead time, regulation or availability.
...
Consume the Deep Navy Footer from Global Chrome V0.5 unchanged. Request a Quote remains visible and points to `/request-a-quote/`. The page adds no Contact fallback, page-specific Footer group or local legal/cookie wording.
...
| Actual Destination Country | Destination Country | Select the country; broad market/region text does not satisfy it |
...
| Destination Country empty | `Select a destination country.` |
...
This complete copy is submitted as `DRAFT_FOR_PROJECT_CONTROL_REVIEW` under `CONV-RFQ-G2-FULL-COPY-01`. Requested checkpoint: verify the complete copy and frozen module order, then decide whether `FULL_COPY_AND_MODULE_ORDER_CONFIRMED` may be recorded.
