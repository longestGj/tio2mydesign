...
const preflight = {
  workset_id: 'APP-000-D32-G3-V0.1',
...
(async () => {
  ...
  const capture = async (page, fileName, { fullDocument = false, ...options }, state) => {
    ...
    if (fullDocument) {
      ...
      // correctly returns focus to its trigger at the footer; clear that
      // transient focus and restore the top before the root-element capture
      // so fixed skip/focus affordances are not composited into the document.
      await page.evaluate(() => {
