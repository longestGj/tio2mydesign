...
const htmlPath=path.join(root,'MARKET-BR-EN_GATE4_COMPLETE_VISUAL_V1.0.html');
...
(async()=>{
 ...
 const result={time:new Date().toISOString(),mode:formal?'FORMAL':'PREFLIGHT',browser:await browser.version(),worksetId:'BR-EN-G4-WS-01',freezeId:'BR-EN-G4-V10-SOURCE-01',source:{path:htmlPath.replace(/\\/g,'/'),bytes:fs.statSync(htmlPath).size,sha256:sha(htmlPath)},approvedUpstream:{path:upstreamPath.replace(/\\/g,'/'),bytes:fs.statSync(upstreamPath).size,sha256:sha(upstreamPath)},viewports:[],assets:[],findings:[]};
