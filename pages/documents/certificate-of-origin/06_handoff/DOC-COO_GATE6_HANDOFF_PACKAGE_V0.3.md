# DOC-COO Gate 6 Handoff Package V0.3

Date: 2026-09-08. Package ID `COO-G6-HANDOFF-03`. Status: `DRAFT_FOR_PROJECT_CONTROL_REVIEW / READY_FOR_REVIEW`. This is the single V0.3 development-delivery candidate and targeted F05 repair; V0.2 is preserved unchanged. B V0.2 remains the sole Buyer Clean text source. This package does not replace the page Manifest or authorize external handoff, Gate 8, development, deployment, publication or indexing.
...
7. Gate 6 execution is authorized by `G6-4PAGE-PRACTICE-01`; `COO-G6-DELIVERY-TR-02` closed F01–F04 but returned `CHANGES_REQUIRED` on F05. V0.3 requires a different-identity targeted recheck and controller closure.
8. `docs/page-playbooks/DOCUMENT_REACH_COO_POST_TDS_EXECUTION_CHECKLIST_V0.1.md` is `USER_APPROVED / ACTIVE`; §§5–6 are the direct authority for the RED/GREEN and complete development-identity hard contract below.
...
### Gate 8 red/green and development identity hard contract

Derived directly from `DOCUMENT_REACH_COO_POST_TDS_EXECUTION_CHECKLIST_V0.1.md` §§5–6, `COO-G9-15` binds the following evidence as a mandatory Gate 8 development receipt, not an optional note or a Gate 9-only request:

1. **Pre-work identity, before any implementation edit:** record the repository identity and absolute repository root, worktree path, branch name (or explicit detached state), full pre-work HEAD SHA, and clean/dirty state. The state evidence must include the exact identity/status commands and outputs. If dirty, inventory every path and distinguish unrelated pre-existing changes from authorized task changes before editing.
2. **RED before implementation:** add or modify the page-owned tests for the approved DOC-COO contract before implementation. Run the focused command against the pre-implementation behavior and record the exact command, mapped Gate 9 IDs, test paths plus content hash or diff identity, exit code and relevant failure output. The failure must demonstrate the missing/violated approved behavior. Test syntax errors, missing dependencies, configuration failure, unrelated suite failure or an assertion that cannot fail for the contract do not count as RED.
3. **GREEN after implementation:** run the same identified tests and focused command after the implementation change and record the exact command, exit code and passing output. The RED and GREEN records must be traceable to the same test identity and acceptance mapping; replacing or weakening the assertion to obtain GREEN is a failure.
4. **Final identity:** record the same repository, worktree and branch, the full final implementation commit SHA, and final clean/dirty state. The final commit must contain the delivered implementation and tests. A clean report includes exact status output; a dirty report inventories every remaining path, classifies task-related versus unrelated pre-existing changes, and explains why the delivered commit remains reproducible. Uncommitted task implementation or tests cannot be the delivered result.
5. **Gate 9 reconstruction:** the receipt maps pre-work HEAD → final commit, identifies the exact test/build environment, and provides enough command/output evidence for Gate 9 to verify the commit exists, inspect the range and rerun the GREEN suite. Missing any identity field, RED/GREEN half, same-test trace, or dirty-path inventory leaves `COO-G9-15` unpassed.
...
| `COO-G9-15` | Gate 8 receipt with repository, worktree, branch, pre-work HEAD, final commit, pre/final clean-or-dirty evidence, and approved-contract RED/GREEN records | Contract tests demonstrably fail before implementation and the same tests pass after it; page-owned focused suite, typecheck, relevant lint and Malaysia production build pass; the final commit is reproducible and full-repository failures are separately attributed | Exact identity/status commands and outputs; repository/root, worktree, branch, full pre-work HEAD and final commit SHAs; dirty-path inventories/classification where applicable; RED/GREEN command, mapped IDs, test paths/hash or diff identity, exit codes and relevant output; typecheck/lint/build commands, first relevant errors and build route manifest | Any missing identity field, non-contract RED, changed/weakened test, absent GREEN, uncommitted task change, unexplained dirty path, page/shared failure or incomplete required command blocks. Unrelated failures require owner/evidence and cannot replace or waive page checks; Gate 8 owns the receipt and Gate 9 independently verifies it. |

The focused suite name is `DOC-COO-G9-FOCUSED`. Gate 8 adds its failing tests before implementation and binds them to actual repository-relative tests covering content/public DTO, action context, initial/Back/Forward/direct revisit, metadata/Schema, fixture eligible, real fail closed, negative token/output scans and `tio2-my` isolation. Gate 9 checks the recorded repository/worktree/branch/pre-work HEAD/final commit/status chain, reruns the exact GREEN command against the delivered final commit and independently inspects the browser result. It also runs the actual repository typecheck, relevant lint command and Malaysia production build; this package does not guess command names before repository inspection.

## 13. Targeted Finding responses

`COO-G6-DELIVERY-TR-02` accepted the V0.2 handling of F01–F04 and `COO-G6-F01`; V0.3 changes only F05 and its adjacent `COO-G9-15`/development-receipt mapping.

| Finding | V0.3 delivery response | Status |
|---|---|---|
| `COO-G6-IR-F01` | V0.2 isolated execution identity/source order remains unchanged | `INHERITED_CLOSED_FROM_COO-G6-DELIVERY-TR-02` |
| `COO-G6-IR-F02` | V0.2 runtime, visual, accessibility and engineering coverage remains unchanged | `INHERITED_CLOSED_FROM_COO-G6-DELIVERY-TR-02` |
| `COO-G6-IR-F03` | V0.2 standing-closure wording remains unchanged | `INHERITED_CLOSED_FROM_COO-G6-DELIVERY-TR-02` |
| `COO-G6-IR-F04` | V0.2 shared-Schema owner boundary remains unchanged | `INHERITED_CLOSED_FROM_COO-G6-DELIVERY-TR-02` |
| `COO-G6-F01` | Delivery mapping remains accepted; actual output remains for Gate 9 | `ADDRESSED_AT_DELIVERY_CONTRACT_LEVEL / PRODUCTION_VERIFICATION_REMAINS_GATE9` |
| `COO-G6-IR-F05` | §12 makes pre-implementation contract RED, same-test post-implementation GREEN, and repository/worktree/branch/pre-work HEAD/final commit/pre-final clean-or-dirty identity mandatory evidence for `COO-G9-15`; invalid or incomplete evidence blocks | `AUTHOR_REPAIR_COMPLETE / PENDING_V0.3_TARGETED_RECHECK` |
...
Status: `READY_FOR_REVIEW`. A different identity must run a `TARGETED_RECHECK` of F05 and adjacent `COO-G9-15`/development-receipt mapping against this exact V0.3 package and execution report. Only after `REVIEW_PASS`, closure of all required Findings and controller identity checks may project control apply `G346-DELEGATED-CLOSURE-20260907` with `approval_basis=USER_STANDING_AUTHORITY` and record `APPROVED / CLOSED` plus `APPROVED_FOR_HANDOFF`. Routine user page review is then `NOT_REQUIRED_BY_STANDING_AUTHORITY`; this does not say the user personally reviewed this package. `HANDED_OFF=NO`; external sending, Gate 8, development, deployment, publication, DNS and indexing remain unauthorized.
