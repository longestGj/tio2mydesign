...
| 连续授权有效但缺一次常规用户点击 | 3/4/6按本站已核实的常设授权（如有）关闭并继续授权内工作 | 重复索取已有批准 |
...
文章任务按[文章交接合同](../../../contracts/article-handoff.md)从原选题计划、Brief、派发、接收和用户批准记录恢复，以article_id排除重复任务。不得强制套用页面Manifest。有效证据与审查链复用；问题退回写作侧具体责任方，不重建整套研究/专业审稿。

沿用逐题计划，用户明确授权的顺序例外记录在原任务中，不推广到其他文章。文章完成和内容批准后核对Gate 1准入与启动授权；缺授权只做准备，已有授权且无暂停按范围推进。页面/正文历史均保留，不为接入总控重做成果。

## 网站准备恢复

按[准备合同](../../../contracts/WEBSITE_PREPARATION_HANDOFF_CONTRACT_V1.6.md)以site_id和preparation_scope恢复原实例。先复用当前来源，定位W1–5差额；接收专业结果与独立链，再记录用户决定和对应接收包/阶段的就绪范围。网站基线只保存引用。无授权不启动页面；新增页面不重跑整站准备；并行工作继续按原授权和实际槽位执行。
