...
总控入口：[Website Planning Orchestrator V0.4](project-orchestrator/agent.md)，由当前根任务承担；[批准、生效及精确身份](../docs/architecture/PROJECT_ORCHESTRATOR_ACTIVATION_V1.3.md)。配套SOP、交接合同和Gate路由在角色中链接；不是新增上级任务或运行注册。
...
内链专项：[网站内链审查 Agent V0.1](internal-link-review/agent.md)。按[生效与交接合同](../docs/architecture/INTERNAL_LINK_REVIEW_ACTIVATION_V1.0.md)由总控派发；不新增Gate、不自行修复或批准发布。项目源未注册、未实测。
