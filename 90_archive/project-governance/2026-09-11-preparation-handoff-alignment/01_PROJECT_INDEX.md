...
总控编排当前入口：[角色V0.12](agents/project-orchestrator/agent.md)及[生效与身份V1.11](docs/architecture/PROJECT_ORCHESTRATOR_ACTIVATION_V1.12.md)。用于状态恢复、准入、派发、接收、异常及关闭；按当前交接合同派发新Gate 5审查与Gate 6差异路径。
...
生产准备与批次安排：[Agent V0.1](agents/production-preparation/agent.md)。W5六部分、三类交付、首批准入接口；只形成计划，不派发或关闭Gate，未实战试跑。
