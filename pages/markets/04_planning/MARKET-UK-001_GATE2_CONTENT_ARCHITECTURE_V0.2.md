# MARKET-UK-001 Gate 2 Content Architecture and Full Copy V0.2
...
| Current Gate 2 state | `DRAFT_FOR_PROJECT_CONTROL_REVIEW` |
| Full copy checkpoint | `FULL_COPY_AND_MODULE_ORDER_CONFIRMED = NOT_YET_GRANTED` |
| Gate 3–10 | `NOT_STARTED / NOT_AUTHORIZED` |
| Buyer Clean language | English |
| Site scope | `tio2-my` |

This document contains the complete Gate 2 content skeleton and the proposed Buyer Clean English copy. It is not approved public copy, a route-live statement, a development handoff or authorization for Gate 3.
...
```text
UK supplier intent
→ Malaysia-origin supply and representative product discovery
→ Great Britain or Northern Ireland destination decision
→ application, specification and importer review
→ document and COO request
→ current classification and trade check
→ qualified RFQ
```

The page is a commercial procurement landing page. Regulatory and trade content helps the buyer organise review; it does not turn the page into legal advice or a trade-policy article.
...
| 2 | `HERO` | `Malaysia-Origin Titanium Dioxide for United Kingdom Buyers` | Understand the supply proposition and next step | `Request a Quote` |
| 3 | `DIRECT_ANSWER` | `A Practical Starting Point for UK Procurement` | Understand what to establish first | `Explore Representative Grades` |
| 4 | `REPRESENTATIVE_GRADES` | `Representative Grades to Begin Your Review` | Enter the global product/application discovery path | Six grade links + Products Hub |
| 5 | `GB_NI_DECISION` | `Where Will the Goods Be Placed on the Market?` | Separate the GB and NI review paths | HSE source links |
| 6 | `PROCUREMENT_CHECKLIST` | `Build a Complete UK Supply Request` | Assemble technical, destination and commercial inputs | `Review Applications` |
| 7 | `DOCUMENTS` | `Prepare the Information Your Team Needs` | Identify review documents and request COO | `Request Documents` |
| 8 | `ORIGIN` | `How Malaysia Origin Fits the Review` | Place the approved origin fact in supplier and import review | `About TiO2 Malaysia` |
| 9 | `TRADE` | `Check Current UK Import Requirements` | Separate origin from classification and current measures | Conditional Trade Update link |
| 10 | `BUYER_QUESTIONS` | `Questions From UK Procurement Teams` | Resolve common sourcing questions | Contextual links |
| 11 | `FINAL_RFQ` | `Prepare Your United Kingdom Supply Request` | Send the information needed for review | `Request a Quote` |
| 12 | `GLOBAL_FOOTER` | Shared Footer | Close with site navigation and persistent RFQ | Shared links |
...
`Evaluate a Malaysia-origin titanium dioxide supply option for UK industrial applications. Begin with representative grade and application paths, identify whether the destination is Great Britain or Northern Ireland, then prepare the product, document and commercial details for review.`

Actions:

1. Primary button: `Request a Quote`
2. Secondary button: `Explore Representative Grades`
...
`A Practical Starting Point for UK Procurement`

Body:

`Start with the intended application, specification or known grade. Review the representative product paths, then identify whether the goods will be placed on the market in Great Britain or Northern Ireland. That destination affects the regulatory and import checks your team should complete before requesting a quotation.`
...
### Module 4 — Representative Grades

Heading:

`Representative Grades to Begin Your Review`

Intro:

`Use these six representative grades to begin your review by application, then compare the grade information with your formulation, specification and document requirements.`

#### Coatings paths

Intro:

`Use these grade pages when coatings is the application entering technical review.`

| Grade | Visible application label | Supporting copy | Action |
|---|---|---|---|
| `M-350` | `Coatings` | `Review the M-350 grade page against your formulation, specification and document requirements.` | `Review M-350` |
| `M-510` | `Coatings` | `Review the M-510 grade page against your formulation, specification and document requirements.` | `Review M-510` |
| `M-896` | `Coatings` | `Review the M-896 grade page against your formulation, specification and document requirements.` | `Review M-896` |

#### Plastics and masterbatch paths

Intro:

`Use these grade pages when plastics or masterbatch is the application entering technical review.`

| Grade | Visible application labels | Supporting copy | Action |
|---|---|---|---|
| `M-200` | `Plastics` · `Masterbatch` | `Review the M-200 grade page against your processing, specification and document requirements.` | `Review M-200` |
| `M-108` | `Plastics` · `Masterbatch` | `Review the M-108 grade page against your processing, specification and document requirements.` | `Review M-108` |
| `M-210` | `Plastics` · `Masterbatch` | `Review the M-210 grade page against your processing, specification and document requirements.` | `Review M-210` |
...
### Module 5 — Great Britain or Northern Ireland
...
`Select the destination so your team can follow the relevant regulatory and import review path.`
...
`EU REACH continues to apply in Northern Ireland. Confirm the importing entity, applicable EU requirements and the product information needed for the intended use and transaction.`

Source link:

`Check Northern Ireland REACH — HSE`

### Module 6 — Procurement Checklist
...
`A useful supplier review connects the product requirement, destination and commercial request. Prepare the following information before your team asks for a quotation.`

#### 1. Define the intended application

`State the formulation or end-use context, such as coatings, plastics or masterbatch, and identify the performance questions your technical team needs to review.`
...
`Identify the technical, safety, quality, Certificate of Analysis, origin and supplier-qualification information your team wants to review.`

#### 5. Add the commercial requirement

`Provide the destination, estimated quantity, required timing and packaging needs so the commercial request can be reviewed in context.`
...
`Prepare the Information Your Team Needs`

Intro:

`Document requirements depend on the grade, intended use, importing entity and review purpose. Tell us what your team needs so the applicable scope can be reviewed.`
...
`Identify the safety, handling and storage information required for workplace and product review.`
...
2. Secondary: `Review Documents and Compliance`

Supporting line:

`We will review the requested documents against the selected grade, intended use, importing entity and shipment context.`

### Module 8 — Malaysia Origin

Heading:

`How Malaysia Origin Fits the Review`

Body:

`Malaysia-origin titanium dioxide gives your team a clear origin fact to carry into supplier qualification and import review. Use the About page to understand the company and the role of Malaysia origin in the wider supply context.`
...
### Module 9 — UK Import and Trade Check
...
`TiO2 Malaysia provides Malaysia-origin titanium dioxide product information, document-request paths and a structured quotation process for UK buyers. Begin with the intended application and destination, then share the specification and commercial details required for review.`

#### Which grades can a UK buyer review first?

`The page presents M-350, M-510 and M-896 for the global Coatings path, and M-200, M-108 and M-210 for the global Plastics and Masterbatch paths. Use them to begin product review, then compare the grade information with your own formulation and specification requirements.`

#### Why must we distinguish Great Britain from Northern Ireland?

`UK REACH applies in Great Britain, while EU REACH continues to apply in Northern Ireland. The destination therefore changes the regulatory and tariff review path. Confirm the applicable role and requirements for the transaction.`

#### What documents can our team request?

`Your request can identify technical and product information, safety documentation, quality and COA information, and origin or supplier-qualification information. Include the selected grade, intended use and shipment context so the requested document scope can be reviewed.`

#### How should Malaysia origin be used in the UK import review?

`Treat Malaysia origin as one product fact in the import review, then verify classification, duty, preference, exclusions, exemptions and current trade-remedy treatment for the specific transaction using official UK sources.`

#### What should we include in a quotation request?

`Include the application, grade or specification when known, Great Britain or Northern Ireland destination, estimated quantity, required timing, packaging needs and the documents your team wants to review.`
...
`Prepare Your United Kingdom Supply Request`

Body:

`Share the application, grade or specification, Great Britain or Northern Ireland destination, estimated requirement, timing, packaging needs and document questions. The request will be reviewed in the relevant product and market context.`
...
| 2 | `Explore Representative Grades` | `#representative-grades` | Move from Hero to the six-grade set | Page anchor |
| 2 | `View All Titanium Dioxide Grades` | `/products/` / `PRODUCT-000` | Continue to the complete range | Required final-site path |
| 2 | `Review Applications` | `/applications/` / `APP-000` | Continue by use-case | Required final-site path |
| 2 | `Review Documents and Compliance` | `/documents/` / `DOC-000` | Understand document categories and request path | Required final-site path |
| 3 | `About TiO2 Malaysia` | `/about/` / `ABOUT-001` | Review entity and origin context | Required final-site path |
| 3 | `Review M-350` | `/products/m-350/` / `GRADE-M350` | Grade discovery | Required final-site path |
| 3 | `Review M-510` | `/products/m-510/` / `GRADE-M510` | Grade discovery | Required final-site path |
| 3 | `Review M-896` | `/products/m-896/` / `GRADE-M896` | Grade discovery | Required final-site path |
| 3 | `Review M-200` | `/products/m-200/` / `GRADE-M200` | Grade discovery | Required final-site path |
| 3 | `Review M-108` | `/products/m-108/` / `GRADE-M108` | Grade discovery | Required final-site path |
| 3 | `Review M-210` | `/products/m-210/` / `GRADE-M210` | Grade discovery | Required final-site path |
...
| Meta description | `Evaluate Malaysia-origin titanium dioxide for UK applications. Review representative grades, GB or Northern Ireland requirements, documents and RFQ steps.` |
...
The 154-character Gate 2 Meta refines the longer Gate 1 direction without changing its intent or approved fact scope.
...
| `UK-GEO-A03` | Why distinguish GB and NI? | `UK REACH applies in Great Britain, while EU REACH continues to apply in Northern Ireland, so the destination changes the regulatory review path.` | GB/NI / Buyer Questions | HSE, checked 2026-09-05 |
...
| `LocalBusiness` | Prohibited | Would imply a UK local establishment | Always omit |
| `Offer` / inventory / delivery markup | Prohibited in current baseline | No approved price, stock, availability or delivery claim | Omit |
...
- descriptive action labels such as `Review M-350`, not repeated ambiguous `Learn more` links;
...
- [x] H1, Hero, all module headings and complete English copy are present.
...
- [x] GB and NI regulatory paths use current official HSE sources.
...
- [x] Gate 3–10, other Market child pages and development remain unauthorized.

## 14. Decisions submitted to project control

Project control is asked to review the following visible package before user presentation:

1. H1: `Malaysia-Origin Titanium Dioxide for United Kingdom Buyers`.
2. Hero: Malaysia-origin UK supply evaluation, representative grades, GB/NI destination and RFQ preparation.
3. Module order: product discovery before the early secondary GB/NI decision.
4. Six exact representative Grade→Application paths and zero UK-specific recommendations.
5. Exact COO statement: `A Certificate of Origin is available upon request.`
6. Neutral UK importer/regulatory copy and conditional trade-status paragraph.
7. Six Buyer Questions, final RFQ copy and complete SEO/GEO/Schema direction.

No additional user fact decision is required for this draft. Project control may identify content revisions, but only the user can approve Gate 2 and set `FULL_COPY_AND_MODULE_ORDER_CONFIRMED`.
...
### 15.1 Complete Buyer Clean sentence revision register
...
### 15.2 Buyer Clean negative-sentence scan
...
- Result after V0.2 revision: `0` sentences beginning with or built around `No`, `not`, `does not`, `cannot`, `never`, `without` or `unless`.
