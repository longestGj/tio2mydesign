...
| RFQ | `pages/conversion/request-a-quote/06_handoff/CONV-RFQ_GATE7_MANIFEST_V1.2.md`；字段/状态/预填取其继承的`CONV-RFQ_GATE7_CMS_API_COMPONENT_MAPPING_V1.0.md`，owner修订取`CONV-RFQ_GATE7_HANDOFF_PACKAGE_V1.1.md`；阶段/环境另消费`pages/conversion/request-a-quote/05_review/CONV-RFQ_GATE8_RECEIVER_ENVIRONMENT_BLOCKER_RECORD_V1.0.md`，不得用历史handoff状态推断尚未开发 |
...
|---|---|---|---|
| Shared Header | Chrome导航与生产Logo引用，current=Markets | 固定RFQ；每表面至多一个aria-current；存在可访问导航时该导航唯一Markets current，compact菜单关闭时可访问导航/current可为0；隐藏表面不聚焦，不为凑计数暴露隐藏导航；无CURRENT文字 | Home/Global Chrome；Visual全页/菜单 |
...
| ES-DEP-02 RFQ | C§3.3；CONV-RFQ + verified receiver operational owner；RFQ环境阻塞记录V1.0 | 接受Spain可改值/来源并保护已有值、正常/无效/失败/重试。D23在2026-09-04已有外部开发记录（reported commit 616193f），route历史探测200、receiver为UNAVAILABLE_IN_CURRENT_ENVIRONMENT，RFQ_RECEIVER_ENVIRONMENT_RELEASE_BLOCKER=OPEN；G9尚待生产等价结果。这是继承日期及报告，不是本轮live探测或Gate8完成证明；旧handoff未开工措辞不能覆盖后续事实 | ES-G9-04/06；route 200不证明接收，owner须给实际环境/接收/失败恢复与隐私一致证据；未成立不放行路径/发布；Spain不实现RFQ |
