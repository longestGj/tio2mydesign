# PRODUCT-PROC-CL Gate 6 Handoff Package V0.3

2026-09-08 · Package ID `CL-G6-HANDOFF-03` · `READY_FOR_REVIEW / NOT_APPROVED / NOT_SENT`

## 1. Single receiving entry and authority

This is the sole V0.3 Gate 6 delivery candidate for `PRODUCT-PROC-CL`, EN, `/products/chloride-process-titanium-dioxide/`, `site_scope=tio2-my`. It is a source-bound implementation and acceptance specification, not a second page Manifest or Buyer Copy source. V0.3 is the narrow repair of V0.2 required by `PRODUCT-PROC-CL-G6-TR-02`; V0.2 remains unchanged and is comparison evidence only.
...
| Execution report | `05_review/PRODUCT-PROC-CL_GATE6_EXECUTION_REPORT_V0.3.md`; execution self-check only |
...
The B-03 list is one continuous, unranked set. The registered Page ID is the stable relationship identity; Grade name/model code, URL and position are separate validated attributes. Implementers must consume the Page ID supplied by the approved relation record and must never derive or guess it from the model code, display name or URL. Exact summaries come only from B V0.2 and may not be independently edited here.

| Registered Page ID | Grade name / model code | Position | Exact clean URL | B V0.2 exact summary | Summary source |
|---|---|---:|---|---|---|
| `GRADE-M350` | `M-350` | 1 | `/products/m-350/` | Excellent hue and high gloss with strong hiding power. | B V0.2 B-03, lines 50–54 |
| `GRADE-M510` | `M-510` | 2 | `/products/m-510/` | TMP/TME-free multi-application grade with high brightness and durability. | B V0.2 B-03, lines 56–60 |
| `GRADE-M896` | `M-896` | 3 | `/products/m-896/` | Superior weather resistance with high gloss and excellent opacity for demanding exterior coatings. | B V0.2 B-03, lines 62–66 |
| `GRADE-M895` | `M-895` | 4 | `/products/m-895/` | High-opacity, high-gloss coatings grade with good weather resistance. | B V0.2 B-03, lines 68–72 |
| `GRADE-M200` | `M-200` | 5 | `/products/m-200/` | High-durability exterior plastics grade with strong anti-chalking performance. | B V0.2 B-03, lines 74–78 |
| `GRADE-M210` | `M-210` | 6 | `/products/m-210/` | High hiding power and easy dispersion for polyolefin masterbatch. | B V0.2 B-03, lines 80–84 |
| `GRADE-M340` | `M-340` | 7 | `/products/m-340/` | High whiteness with strong high-temperature anti-yellowing performance. | B V0.2 B-03, lines 86–90 |
| `GRADE-M886` | `M-886` | 8 | `/products/m-886/` | Bright-white plastics grade with excellent dispersion and processability. | B V0.2 B-03, lines 92–96 |

The eight Page IDs and their clean URLs are independently corroborated by Registry V0.2 §4.4 and the matching keyword-master rows. PRODUCT V0.3 supplies the approved Chloride relationship/classification; B V0.2 supplies the exact public summaries and display order. Wording such as “Superior” in the M-896 summary is approved exact copy and does not create a rank.
...
| `CL-SRC-03` | Registry V0.2 §4.4 + keyword-master rows + PRODUCT V0.3 + B B-03 + C §§1,3.3,7: eight fixed Chloride Grades | Each ordered relation explicitly stores the registered Page ID, separate Grade name/model code, exact B summary, clean same-scope Grade URL and approved position; Page ID is never inferred from name/model/URL | Exactly eight complete records render in the fixed order; API/read-back retains all five attributes; ordinary links remain crawlable | `CL-G9-03`, `CL-G9-07`, `CL-G9-08`, `CL-G9-12` |
...
| Grade directory | Eight relation records, each with required `registered_page_id`, separate `grade_name_or_model_code`, fixed `position`, exact `clean_url` and exact B V0.2 `summary` | Exactly the eight §2 Page IDs, each unique and bound to its approved four attributes. Missing/unknown/duplicate Page ID, Page-ID/name mismatch, Page-ID/URL mismatch, summary mismatch, position mismatch or derivation of Page ID from name/URL is invalid. The editor selects an existing registered Grade Page ID; the system reads back the stored ID and its bound attributes. |
...
|---|---|---|---|
| Positive same-scope payload | Resolve the current WordPress-derived `PRODUCT-PROC-CL` payload in `tio2-my`; each Grade relation supplies an explicit registered Page ID plus separate name/model, position, clean URL and B summary; validate the exact §2 tuples and render exact content, order, actions, shared shell, head and Schema | Page-private hardcoded/static content as the production content authority, client-only hidden copy, screenshot-only text, inferred Grade relationships or a Page ID synthesized from model/URL | WordPress edit/read-back of all five relation attributes + CMS/API trace showing explicit Page IDs + SSR/rendered DOM/head/JSON-LD + route response |
...
| Registered Page ID missing/extra/duplicate/unknown | Reject the invalid required relation collection; do not expose a seven-item, nine-item or mixed list | Partial list, hidden missing row, auto-imported replacement/extra Grade, or recovering identity by parsing model code/URL | Negative fixtures for absent, extra, duplicate and unknown explicit Page IDs; CMS/API read-back plus visible/JSON-LD cardinality checks |
| Page ID attribute mismatch | Reject any tuple where a registered Page ID is paired with the wrong model/name, position, URL or exact B summary | Trusting a familiar model/URL while ignoring the stored Page ID; silently rewriting the Page ID or another attribute | One negative fixture per mismatch class, including `GRADE-M350` paired with M-510 or `/products/m-510/`; controlled failure and no partial/fallback render |
| Payload array reordered | Output may normalize array transport order only by the validated explicit `position` values while the complete registered Page-ID tuples stay unchanged; changed/duplicate/missing positions are rejected | Blind API-array order, ordering by Page ID/model/URL, or editorial drag reorder changing public meaning without approval | Permuted-array fixture preserving valid tuples plus altered/duplicate/missing-position fixtures and Page-ID read-back |
...
| B-03 | Eight View Grade actions | Exact registered Page ID/name/position/URL bindings in §2; browser navigation uses each bound clean URL | Ordinary navigation; Page ID remains the internal relation identity and is not guessed from the target; reading/clicking never selects a Grade elsewhere |
...
- one `ItemList`: `@id = Canonical + #chloride-grade-list`; name equal to B-03 heading; `numberOfItems=8`; `itemListOrder=https://schema.org/ItemListUnordered`; eight `ListItem` entries with positions, exact Grade names and clean URLs matching §2. The source relation for each `ListItem` must read back the corresponding explicit registered Page ID in §2; the Page ID is an internal identity/control field and need not be added to buyer-visible text or Schema when C does not authorize such a field.
...
| `CL-G9-03` | WordPress relation read-back and CMS/API trace contain exactly the eight registered Page IDs in §2, each explicitly bound to the exact model/name, position, clean URL and B summary; the visible directory contains those exact eight bound records in order and no extra Grade. Evidence must show the stored Page ID and may not reconstruct it from model/URL. | Missing/unknown/duplicate Page ID, any attribute mismatch, inference/guessing, omission, reorder, wrong URL/summary or extra Grade fails; page/data owner |
...
| `CL-G9-07` | Correct payload with explicit Page IDs renders; fixtures for missing/error/wrong-scope, missing/extra/duplicate/unknown Page ID and every Page-ID/name/position/URL/summary mismatch produce controlled failure and no fallback/partial public page. At least one fixture proves a plausible model/URL cannot rescue a missing or wrong Page ID. | Any inference, partial/cross-scope/default content or silent tuple repair fails; page/API owner |
| `CL-G9-08` | A permuted transport array with intact explicit Page-ID tuples is normalized only by validated position; changed/missing/duplicate positions fail. Cache keys/tags and invalidation isolate scope/page/contract and preserve the Page-ID tuples through hit, revalidation failure and recovery. | Blind reorder, ordering by model/URL/Page ID, cache mixing, tuple loss or invalid refresh poisoning fails; data/cache owner |
...
| `CL-G9-12` | JSON-LD contains one WebPage, one 3-item BreadcrumbList and one 8-item unordered ItemList with exact IDs/values/order/URLs and shared entity references. A joined read-back maps each ListItem position/name/URL to the same explicit registered Page ID in the WordPress/API relation record, with all eight §2 tuples accounted for exactly once. | Missing/duplicate/wrong node/cardinality/order/ID, joined tuple mismatch or Page ID inferred from Schema URL/name fails; Schema + data owners |
...
| `CL-G9-03` | Registry V0.2 §4.4; matching keyword-master rows; B B-03; C §§1,2,3.3,7.1,8; PRODUCT V0.3 matrix/audit |
...
| `CL-G9-07…09` | Registry V0.2 §4.4; matching keyword-master rows; C §4; Gate 6 execution contract V1.1; governance §14; fixed Grade/source rules in C §§1,3.3,7 |
| `CL-G9-10…11` | C §5 in full |
| `CL-G9-12…13` | Registry V0.2 §4.4; matching keyword-master rows; C §§6–7 in full; B visible content; shared entity owners |
...
## 12. Finding response, V0.2 → V0.3 difference and stop

This package is a narrow V0.3 repair of V0.2 after targeted recheck `PRODUCT-PROC-CL-G6-TR-02`. It preserves every V0.2 contract accepted by that recheck and changes only the relationship-identity portion required for `CL-G6-IR-F02`:

1. §2 now separates and binds registered Page ID, Grade name/model code, position, clean URL and exact B V0.2 summary/source for all eight records.
2. §§3–5 make the registered Page ID the explicit WordPress/CMS/API relationship identity and prohibit deriving or guessing it from model code, name or URL.
3. §5 adds missing/extra/duplicate/unknown Page-ID and Page-ID/attribute mismatch fixtures while retaining all V0.2 positive, empty, error, wrong-scope, reorder and cache behavior.
4. §§8,10–11 require joined Gate 9 read-back from explicit WordPress/API Page ID through the visible Grade row and ItemList position/name/URL.

No B/C text, Grade set/order, URL, prototype, visual asset, shared-owner contract, OG/X condition, source-only action, ItemList boundary, dependency or other stable Gate 9 requirement changes.

| Stable independent-review Finding | Package closure location | Claimed closure object |
|---|---|---|
| `CL-G6-IR-F01` | V0.2 targeted recheck §3; V0.3 preserves separate execution/review identity | `ADDRESSED` in V0.2 recheck; unchanged |
| `CL-G6-IR-F02` | V0.3 §§2–5, 8, 10–11 | `DELIVERY_REPAIR_IMPLEMENTED / PENDING_TARGETED_RECHECK`; production implementation remains unverified |
| `CL-G6-IR-F03` | V0.2 targeted recheck §3; V0.3 §§3 (`CL-SRC-08/09`), 7, 10 (`CL-G9-11`) unchanged | `ADDRESSED` in V0.2 recheck; unchanged |
| `CL-G6-IR-F04` | V0.2 targeted recheck §3; V0.3 §§9–10 (`CL-DEP-A11Y-*`, `CL-G9-16`) unchanged | `ADDRESSED` in V0.2 recheck; Gate 9 evidence remains OPEN as designed |

Existing `CL-G6-F01` was found `ADDRESSED / CLOSED_BY_DELIVERY_MAPPING_IN_V0.2` by the V0.2 targeted recheck. V0.3 preserves §8 plus `CL-G9-12/13` and adds the stable Page-ID joined read-back without expanding Schema. The frozen prototype remains unchanged and lacks JSON-LD; production Schema remains unimplemented and unverified.

Current result: **`READY_FOR_REVIEW / NOT_APPROVED / NOT_SENT`**. A reviewer different from this execution author must target-recheck `CL-G6-IR-F02` against the current registry, keyword master, PRODUCT V0.3 and B V0.2. Gate 6 remains not closed; `APPROVED_FOR_HANDOFF`, actual handoff and Gate 8–10 are not asserted or authorized. No D16, CMS, code, test, deployment, publication, DNS or indexing action occurred.
