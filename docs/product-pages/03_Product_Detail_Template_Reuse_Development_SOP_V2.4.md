# TiO2 Malaysia 型号产品详情页模板复用开发 SOP V2.4
...
> 2026-09-07：继承已批准V2.0的产品合同与双通道要求，按用户“按照你的方案，开始修正。”及[Gate工作流V3.4](../architecture/GATE_WORKFLOW_V3.4.md)同步4/5、6/7合并与阶段边界。V2.0及历史批准包保留。完整JSON交付不省略；语义合同不要求开发另建平行模型。
...
| Supersedes | V2.3的未来阶段流程；V2.0、V2.1及更早合同按当前工作流兼容，历史批准保持 |
...
流程：`Gate 1 → Gate 2 → Gate 4制作 → Gate 5独立审查 → 总控关闭 → Gate 6 → Gate 8 → Gate 9`。

标准通道Gate 4同一Agent完成一致性、结构与视觉4A→4B；最终Gate 5不同作者独立审查及总控依授权关闭一次。新页不要求Gate 3冻结。有效上游按当前工作流复用。
...
`Gate 4 = SATISFIED_BY_APPROVED_TEMPLATE_REUSE`

仅在合并Gate 4记录`SATISFIED_BY_APPROVED_TEMPLATE_REUSE`，引用同一适配判断、模板精确版本与用户快速通道批准；继承有效独立审查；新增或改变范围由Gate 5独立核对，不重复适配报告。这不是免验：当前型号内容密度、三端重排和条件状态仍须验证。旧`Gate 3–5`及`STANDARD_GATE_3_5_REQUIRED`只作历史标识，不回写旧包。
...
| 是否必须重复合并Gate 4 | 精确复用已批准模板且用户明确批准时，可以快速通道满足 |
