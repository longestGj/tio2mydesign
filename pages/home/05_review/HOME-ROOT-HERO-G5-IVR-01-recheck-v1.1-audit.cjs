...
const base = path.join(root,'pages/home/04_planning/visual-designs/home-root-page-hero-v1.2');
const current = path.join(base,'homepage-root-page-hero-preview-v1.2.html');
const old = path.join(root,'pages/home/04_planning/visual-designs/home-root-page-hero-v1.1/homepage-root-page-hero-preview-v1.1.html');
...
const git = p => execFileSync('git',['show','2492cb0:'+path.relative(root,p).split(path.sep).join('/')],{cwd:root,maxBuffer:20000000});
const html=fs.readFileSync(current,'utf8');
const body=s=>s.match(/<body>[\s\S]*<\/body>/)[0];
const result={review_id:'HOME-ROOT-HERO-G5-IVR-01',date:new Date().toISOString(),frozen_commit:execFileSync('git',['rev-parse','2492cb0'],{cwd:root,encoding:'utf8'}).trim(),node:process.version,evidence_class:'ACTUAL_RUNTIME_LOCAL_PROTOTYPE',files:[],dependencies:[],body:{},runtime:[]};
...
for(const [name,p] of [['previous_v1_1',old],['approved_applications_aligned_v1_1',approved]]){const b=body(fs.readFileSync(p,'utf8'));result.body[name]={sha256:hash(b),currentSha256:hash(body(html)),exactBodyEqual:b===body(html)};}
(async()=>{const browser=await chromium.launch({headless:true});result.browser=browser.version();
for(const [width,height] of [[1440,900],[768,1400],[390,1500],[767,1400],[1023,900],[1024,900],[350,900]]){
const page=await browser.newPage({viewport:{width,height},deviceScaleFactor:1,colorScheme:'light',reducedMotion:'reduce'});const failures=[];page.on('requestfailed',r=>failures.push({url:r.url(),reason:r.failure()}));
...
observation.requestFailures=failures;const priorPage=await browser.newPage({viewport:{width,height},deviceScaleFactor:1,colorScheme:'light',reducedMotion:'reduce'});await priorPage.goto(pathToFileURL(old).href);await priorPage.evaluate(()=>document.fonts.ready);observation.previousNonHeroShells=await priorPage.evaluate(()=>[...document.querySelectorAll('main .shell:not(.hero-shell)')].map(e=>{const r=e.getBoundingClientRect();return {module:e.closest('[data-module]')?.dataset.module,x:r.x,width:r.width,height:r.height}}));await priorPage.close();result.runtime.push(observation);await page.close();}
await browser.close();fs.writeFileSync(path.join(__dirname,'HOME-ROOT-HERO-G5-IVR-01-recheck-v1.1-runtime.json'),JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result,null,2));})().catch(e=>{console.error(e);process.exit(1)});
