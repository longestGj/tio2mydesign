# Evidence Gap User Decision Register V1.0
...
| `EG-002` | `A Certificate of Origin is available upon request.` | MARKET-UK-001 visible copy and semantically equivalent machine-readable expression; other pages remain separately governed | `MARKET-UK-001: USER_APPROVED / CLOSED`; `OTHER PAGES: PENDING_USER_FACT_CONFIRMATION` | User approved the exact MARKET-UK-001 statement on 2026-09-05; approval does not extend to every-shipment provision, customs acceptance or a trade outcome | MARKET-UK-001 may consume the exact fact within scope; other page owners must obtain their own applicable decision before use |
